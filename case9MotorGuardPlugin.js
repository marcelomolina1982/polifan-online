...
// Complete-kit safety + resilient network. Productive minimum is a target, not a hard certification blocker.
export default function case9MotorGuardPlugin(){
  return {
    ...
    transform(code,id){
      ...
      // The hardened lab motor has its own certification gate and is transformed by
      // sparrowLabRoutePlugin first. The historical exact-text patch below targets the
      // older pretty-printed MotorDefinitivo and would otherwise fail the build.
      if(code.includes('MIN_CERTIFIED_GAP_MM=3'))return {code,map:null}
