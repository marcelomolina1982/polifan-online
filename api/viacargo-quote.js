...
export default async function handler(req,res){
  ...
  if(!locality||!province)return res.status(400).json({ok:false,error:'Completá localidad y provincia para validar correctamente el destino de Vía Cargo.'})
  try{
    const r=await fetchTimed(BASE+'/quote',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify({destinationCp,quantity,locality,province})},40000)
    ...
    if(!r.ok||!body.ok||!agency)return res.status(r.status===400?400:422).json({ok:false,available:false,error:body.error||'Vía Cargo no devolvió una tarifa Agencia → Agencia segura para esos datos.',destinationCp,locality,province,quantity})
    const price=numericPrice(agency.price)
    res.setHeader('cache-control','no-store')
    return res.status(200).json({ok:true,available:true,carrier:'Vía Cargo',service:'Agencia → Agencia',origin:body.origin||'BOULOGNE (1609) - BUENOS AIRES',destination:body.destination||'',destinationCp,locality,province,quantity,price,priceText:agency.price||'',payment:'destino',package:{kg:Number(body.kg||0),dimensions:body.dimensions||[]},declaredValue:Number(body.declaredValue||100000),quotedAt:new Date().toISOString(),informative:true})
  }catch(e){
