...
must(orders.includes("const isClosed=o=>['Entregado','Cancelado'].includes(o.status)")&&orders.includes('const isPastDelivery=o=>Boolean(o.delivery)&&String(o.delivery)<todayKey()')&&orders.includes('const isOperational=o=>!isClosed(o)&&!isPastDelivery(o)')&&orders.includes('const isHistorical=o=>isClosed(o)||isPastDelivery(o)')&&orders.includes("view==='history'?isHistorical(o):isOperational(o)"),'Pedidos perdió la ventana operativa: hoy/futuro en Pedidos y vencidos/finales en Historial')
...
console.log('PRODUCTION SOURCE GUARDS OK · build verifica ventana operativa, no reescribe')
