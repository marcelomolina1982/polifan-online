...
// La política live fue ampliada después para Para cortar y En corte. Agregamos
// operations sin destruir esas ramas ni depender de una versión histórica exacta.
const appPath='src/AppV2.jsx'
let app=fs.readFileSync(appPath,'utf8')
if(!app.includes("target==='operations'?new Set(['orders','movements','cutBatches'])")){
  const anchor="const liveProductionKeys=target==='sheetplanner'?new Set(['orders','movements','cutBatches']):"
  if(!app.includes(anchor))throw new Error('journey-v2: no se encontró política live actual de producción')
  app=app.replace(anchor,"const liveProductionKeys=target==='sheetplanner'?new Set(['orders','movements','cutBatches']):target==='operations'?new Set(['orders','movements','cutBatches']):")
}
app=app.replace(/v25\.0\.81/g,'v25.0.82').replace(/25\/08\/2026/g,'04/09/2026')
fs.writeFileSync(appPath,app)
if(!app.includes("target==='operations'?new Set(['orders','movements','cutBatches'])"))throw new Error('journey-v2: Centro operativo no quedó con producción fresca')
