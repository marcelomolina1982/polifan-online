...
customer=replaceOnce(customer,"      setViaCargoQuote(normalized);return normalized","      if(requestId===viaCargoRequestRef.current)setViaCargoQuote(normalized)\n      return normalized",'guardado de cotización vigente')
customer=replaceOnce(customer,"      setViaCargoQuote(null);setViaCargoQuoteError(message);if(!silent)alert(message);return null\n    }finally{setViaCargoQuoting(false)}","      if(requestId===viaCargoRequestRef.current){setViaCargoQuote(null);setViaCargoQuoteError(message)}\n      if(!silent)alert(message);return null\n    }finally{if(requestId===viaCargoRequestRef.current)setViaCargoQuoting(false)}",'cierre seguro de cotización')
...
console.log('v25.0.61 PREDEPLOY OK · Motor/Para cortar unificados · Via Cargo protegido contra respuestas viejas')
