...
function replaceOnce(before,after,label){
  if(!s.includes(before))throw new Error('v25.0.57: no encontré '+label)
  const count=s.split(before).length-1
  if(count!==1)throw new Error('v25.0.57: '+label+' aparece '+count+' veces')
...
replaceOnce("  const total = items.reduce((sum, item) => sum + item.qty, 0)+specialQty\n  const deliveryEstimate",`  const total = items.reduce((sum, item) => sum + item.qty, 0)+specialQty
  ...
  const deliveryEstimate`,'cálculo logística')
...
let version=fs.readFileSync(versionFile,'utf8').replace(/APP_VERSION='[^']*'/,"APP_VERSION='25.0.60'").replace(/APP_VERSION_LABEL='[^']*'/,"APP_VERSION_LABEL='v25.0.60'").replace(/APP_VERSION_NAME='[^']*'/,"APP_VERSION_NAME='Polifan 25 · logística GBA/CABA por zonas automática'")
fs.writeFileSync(versionFile,version)
const swFile='public/sw.js';fs.writeFileSync(swFile,fs.readFileSync(swFile,'utf8').replace(/SW_VERSION='[^']*'/,"SW_VERSION='25.0.60'"))
const indexFile='index.html';fs.writeFileSync(indexFile,fs.readFileSync(indexFile,'utf8').replace(/const build='[^']*'/,"const build='25.0.60'"))
console.log('v25.0.60: logística GBA/CABA detecta zona, tarifa y total con envío automáticamente')
