...
function patch(oldText,newText,label){
  ...
  if(!text.includes(oldText)) throw new Error(`v25.0.20 patch: no se encontró ${label}`)
  text=text.replace(oldText,newText)
}

patch("const LAB_STORAGE='polifan-motor-lab-last-plan-v3'","const LAB_STORAGE='polifan-motor-lab-last-plan-v6-sparrow-v20-final-growth'",'LAB_STORAGE')
patch("const ACTIVE_JOB_STORAGE='polifan-motor-lab-active-job-v1'","const ACTIVE_JOB_STORAGE='polifan-motor-lab-active-job-v4-sparrow-v20-final-growth'",'ACTIVE_JOB_STORAGE')

patch("V1.7 certificando…","V1.8 certificando…",'texto de certificación')
patch("const payload={widthCm:121.4,heightCm:58,gapCm:.3,targetDensity:75,kits:industrial.kits}","const payload={widthCm:122,heightCm:58,gapCm:.25,targetDensity:70,kits:industrial.kits,clientBuild:'v25.0.20-final-growth',maxGrowthTarget:16}",'payload Sparrow')
patch("Sparrow + V1.7 · ${plan.units.length} diseños","Sparrow V1.8 · ${plan.units.length} diseños",'nota de placa')
patch('Generar placas · Motor Sparrow + Certificador V1.7','Generar placas · Sparrow V1.8 · Optimizado v25.0.20 · Objetivo 70%+','título')
...
patch('objetivo ≥75%, sin descartar 11/12 válidas','objetivo mínimo ≥70% · gap 2,5 mm · explora 11/12/13/14/15/16 y conserva base 10','criterio productivo')
patch('Sparrow asíncrono · V1.7 certifica','Sparrow V1.8 · runtime final dinámico · crecimiento 11→16 · borde 3 mm','arquitectura')
...
console.log('v25.0.20: runtime dinámico + crecimiento 11..16 + telemetría preparados')
