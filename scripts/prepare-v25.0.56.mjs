...
const quoteBlock=`  function update(field, value) {setData(previous => ({ ...previous, [field]: value }))}\n\n  async function quoteViaCargo({silent=false}={}) {\n    const cp=String(data.postalCode||'').trim()\n    const locality=String(data.locality||'').trim()\n    const province=String(data.province||'').trim()\n    if(!/^\\d{4}$/.test(cp)){if(!silent)alert('Ingresá un código postal de 4 dígitos para cotizar Vía Cargo.');return null}\n    if(!locality||!province){if(!silent)alert('Completá localidad y provincia para cotizar Vía Cargo con seguridad.');return null}\n    if(total<1){if(!silent)alert('Elegí al menos un producto antes de cotizar el envío.');return null}\n    setViaCargoQuoting(true);setViaCargoQuoteError('')\n    try{\n      const response=await fetch('https://viacargo-quote-probe2.onrender.com/api/cotizar',{method:'POST',cache:'no-store',headers:{'content-type':'application/json'},body:JSON.stringify({destinationCp:cp,locality,province,quantity:total})})\n      const payload=await response.json().catch(()=>({}))\n      if(!response.ok||!payload?.ok)throw new Error(payload?.error||'Vía Cargo no tiene una tarifa Agencia → Agencia segura para esos datos.')\n      const price=Number(payload.price||0)\n      if(!Number.isFinite(price)||price<=0)throw new Error('Vía Cargo devolvió una tarifa inválida.')\n      const normalized={...payload,destinationCp:cp,locality,province,quantity:total,price,priceText:payload.priceText||'',package:payload.package||null}\n      setViaCargoQuote(normalized);return normalized\n    }catch(error){\n      const message=error?.message||'No se pudo cotizar Vía Cargo en este momento.'\n      setViaCargoQuote(null);setViaCargoQuoteError(message);if(!silent)alert(message);return null\n    }finally{setViaCargoQuoting(false)}\n  }\n\n  async function send() {`
replaceOnce(updateAnchor,quoteBlock,'función update/send')

replaceOnce("    setSending(true)\n    const latestState=await refreshPlanning(false,{retries:1})","    setSending(true)\n    let finalViaCargoQuote=viaCargoQuote\n    if(data.method==='Vía Cargo'){\n      const currentCp=String(data.postalCode||'').trim()\n      const currentLocality=String(data.locality||'').trim()\n      const currentProvince=String(data.province||'').trim()\n      if(!finalViaCargoQuote||finalViaCargoQuote.destinationCp!==currentCp||String(finalViaCargoQuote.locality||'')!==currentLocality||String(finalViaCargoQuote.province||'')!==currentProvince||Number(finalViaCargoQuote.quantity)!==Number(total)){finalViaCargoQuote=await quoteViaCargo({silent:true})}\n      if(!finalViaCargoQuote){setSending(false);return alert(viaCargoQuoteError||'No pudimos confirmar una tarifa Agencia → Agencia segura de Vía Cargo para esos datos. Contactanos para revisarlo.')}\n    }\n    const latestState=await refreshPlanning(false,{retries:1})",'cotización antes de guardar')
...
replaceOnce("{data.method==='Vía Cargo'&&<label>¿Cómo lo recibís?<select value={data.agencyDelivery} onChange={event=>update('agencyDelivery',event.target.value)}><option>Envío a domicilio</option><option>Retiro en agencia</option></select></label>}","{data.method==='Vía Cargo'&&<div className=\"delivery-estimate-box\"><small>🚚 VÍA CARGO · AGENCIA → AGENCIA</small><b>Se abona al retirar en la agencia de destino</b><span>Origen: Boulogne (CP 1609). Validamos CP, localidad y provincia antes de aceptar el precio.</span><button type=\"button\" className=\"planning-retry\" disabled={viaCargoQuoting||total<1||!/^\\d{4}$/.test(String(data.postalCode||'').trim())||!String(data.locality||'').trim()||!String(data.province||'').trim()} onClick={()=>quoteViaCargo()}>{viaCargoQuoting?'Cotizando…':'Actualizar cotización'}</button>{viaCargoQuote&&<strong>{money(viaCargoQuote.price)} <small>estimado · {viaCargoQuote.destination||data.postalCode}{viaCargoQuote.cached?' · respuesta rápida':''}</small></strong>}{viaCargoQuoteError&&<span>{viaCargoQuoteError} Contactanos para revisarlo.</span>}</div>}",'selector Via Cargo')
...
console.log('v25.0.59: Vía Cargo directo a Render /api/cotizar · sin Vercel por cotización · CP/localidad/provincia validados')