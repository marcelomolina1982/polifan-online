...
}
{
 const file='src/pages/CutBatches.jsx';let src=fs.readFileSync(file,'utf8');if(!src.includes("import {advanceOperationalJourney} from '../lib/customerJourneyOperational'"))src=src.replace(/^(import React[^\n]*\n)/m,m=>m+"import {advanceOperationalJourney} from '../lib/customerJourneyOperational'\n");src=src.replace("    const pending=(db.cutBatches||[]).filter(b=>b.status==='En corte' && String(b.name||'').startsWith('Placa automática Sparrow'))","    const pending=[] // Sparrow espera confirmación manual del corte");src=src.replace(/  async function finish\(batch\)\{[\s\S]*?\n  \}\n\n  async function cancel\(batch\)\{/,`  async function finish(batch){
  ...
  async function cancel(batch){`);src=src.replace('Las placas automáticas de Sparrow pasan a Terminadas al ingresar y suman su producción al inventario. Después podés modificarlas o anularlas y el stock se corrige automáticamente.','Las placas automáticas quedan En corte hasta que confirmes que terminaron. Recién ahí suman su producción al inventario y comienza el reloj de 3 horas.');fs.writeFileSync(file,src)
}
{
...
console.log('CUSTOMER JOURNEY INTEGRATION OK · corte manual + reloj 3 h')
