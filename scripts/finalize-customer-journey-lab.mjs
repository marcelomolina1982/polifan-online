...
{
 const file='src/pages/CutBatches.jsx';let src=fs.readFileSync(file,'utf8');if(!src.includes("import {advanceOperationalJourney} from '../lib/customerJourneyOperational'"))src=src.replace(/^(import React[^\n]*\n)/m,m=>m+"import {advanceOperationalJourney} from '../lib/customerJourneyOperational'\n");if(!src.includes("import {inventoryMovements}"))src=src.replace("import { today } from '../lib/format'","import { today } from '../lib/format'\nimport {inventoryMovements} from '../lib/inventory'");src=src.replace("const pending=(db.cutBatches||[]).filter(b=>b.status==='En corte' && String(b.name||'').startsWith('Placa automática Sparrow'))","const pending=[]");src=src.replace(/  async function finish\(batch\)\{[\s\S]*?\n  \}\n\n  async function cancel\(batch\)\{/,`  async function finish(batch){
