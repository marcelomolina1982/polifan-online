...
# - busca una base REAL de 10;
# - certifica >=3 mm antes de conservarla;
# - con 10 segura prueba pocas alternativas de 11;
# - evita las búsquedas largas de varios minutos.
MAX_POOL = 64
BASE_CANDIDATES = 4
ELEVEN_CANDIDATES = 3
BASE_BUDGET = 6.0
...
def fast_certified_nest():
    ...
    for idx, (label, selected) in enumerate(groups10):
        if time.time() - started >= NOMINAL_LIMIT_SECONDS:
            break
        result = ns._run_sparrow(selected, gap, BASE_BUDGET, SEEDS[idx % len(SEEDS)], continuous=idx > 0)
    ...
    if base is None:
        return jsonify(ok=False, error='FAST-CERT no encontró 10 certificadas en la búsqueda corta.', engine='Motor Lab FAST-CERT', attempts=attempts, elapsedSeconds=round(time.time()-started, 1), requiredGapMm=3.0), 422

    best_selected, best_result, best_cert = base

    # Sólo tres intentos de 11. Si no mejora rápido, se devuelve inmediatamente la base 10.
    groups11 = smart._priority_safe_candidates(kits, 11, ELEVEN_CANDIDATES)
    for idx, (label, selected) in enumerate(groups11):
        if time.time() - started >= NOMINAL_LIMIT_SECONDS:
            break
        result = ns._run_sparrow(selected, gap, ELEVEN_BUDGET, 429 + idx * 127, continuous=True)
        valid, cert = _certified(selected, result)
    ...
    response = ns._result_payload(best_selected, f'Motor Lab FAST-CERT: {len(best_selected)} completas · prioridad protegida', best_result, kits, rejected, attempts, started, None)
    payload = response.get_json()
    if not isinstance(payload, dict) or not payload.get('ok'):
        return response
    payload.update({
        'engine': 'Motor Lab FAST-CERT · 10 segura + intento corto de 11',
