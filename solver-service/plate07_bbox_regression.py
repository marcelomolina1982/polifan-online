...
from sa_runtime_wrapper import solve_v4_sa as solve_v4
...
def run():
    ...
    try:
        ...
        result={'marker':'POLIFAN_PLACA07_BBOX','httpStatus':status,'ok':bool(isinstance(data,dict) and data.get('ok')),'completeFigures':data.get('completeFigures') if isinstance(data,dict) else None,'candidatePool':data.get('candidatePool') if isinstance(data,dict) else None,'batchAccepts':data.get('batchAccepts') if isinstance(data,dict) else None,'rescueRounds':data.get('rescueRounds') if isinstance(data,dict) else None,'residualAttempts':data.get('residualAttempts') if isinstance(data,dict) else None,'cavityAccepted':data.get('cavityAccepted') if isinstance(data,dict) else None,'saEscapeRan':data.get('saEscapeRan') if isinstance(data,dict) else None,'saIterations':data.get('saIterations') if isinstance(data,dict) else None,'saAcceptedWorse':data.get('saAcceptedWorse') if isinstance(data,dict) else None,'saSuccess':data.get('saSuccess') if isinstance(data,dict) else None,'saCertified':data.get('saCertified') if isinstance(data,dict) else None,'saCertificationReason':data.get('saCertificationReason') if isinstance(data,dict) else None,'pairAccepted':data.get('pairAccepted') if isinstance(data,dict) else None,'swapAccepted':data.get('swapAccepted') if isinstance(data,dict) else None,'gapMm':data.get('gapMm') if isinstance(data,dict) else None,'widthCm':data.get('widthCm') if isinstance(data,dict) else None,'heightCm':data.get('heightCm') if isinstance(data,dict) else None,'attemptCount':len(data.get('attempts') or []) if isinstance(data,dict) else 0,'error':data.get('error') if isinstance(data,dict) else 'invalid','elapsedSeconds':round(time.time()-started,2)}
