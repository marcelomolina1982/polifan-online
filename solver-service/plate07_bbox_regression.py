...
def run():
    ...
    try:
        ...
        result={'marker':'POLIFAN_PLACA07_BBOX','httpStatus':status,'ok':bool(isinstance(data,dict) and data.get('ok')),'completeFigures':data.get('completeFigures') if isinstance(data,dict) else None,'candidatePool':data.get('candidatePool') if isinstance(data,dict) else None,'batchAccepts':data.get('batchAccepts') if isinstance(data,dict) else None,'rescueRounds':data.get('rescueRounds') if isinstance(data,dict) else None,'residualAttempts':data.get('residualAttempts') if isinstance(data,dict) else None,'pairAccepted':data.get('pairAccepted') if isinstance(data,dict) else None,'swapAccepted':data.get('swapAccepted') if isinstance(data,dict) else None,'gapMm':data.get('gapMm') if isinstance(data,dict) else None,'widthCm':data.get('widthCm') if isinstance(data,dict) else None,'heightCm':data.get('heightCm') if isinstance(data,dict) else None,'attemptCount':len(data.get('attempts') or []) if isinstance(data,dict) else 0,'error':data.get('error') if isinstance(data,dict) else 'invalid','elapsedSeconds':round(time.time()-started,2)}
