...
from motor_definitivo_v1 import solve_svg_text

@app.get('/motor-definitivo/health')
def motor_definitivo_health():
    return jsonify(ok=True,engine='Motor Polifan Definitivo V1',mode='test',preferredGapMm=3.0,absoluteMinGapMm=2.5)
...
def motor_definitivo_svg():
    ...
    try:
        seconds3=max(1.0,min(20.0,float(data.get('seconds3') or 6.0)))
        seconds25=max(1.0,min(30.0,float(data.get('seconds25') or 10.0)))
        result=solve_svg_text(svg_text,filename,seconds3,seconds25)
        certified=str(result.get('status','')).startswith('CERTIFICADO')
        return jsonify(ok=certified,engine='Motor Polifan Definitivo V1',**result),(200 if certified else 422)
    except Exception as exc:
        return jsonify(ok=False,error=str(exc),engine='Motor Polifan Definitivo V1'),500
