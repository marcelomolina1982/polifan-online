...
from motor_definitivo_v3 import solve_svg_text

@app.get('/motor-definitivo/health')
def motor_definitivo_health():
    return jsonify(ok=True,engine='Motor Polifan Definitivo V1.3',mode='test',preferredGapMm=3.0,absoluteMinGapMm=2.5)
...
def motor_definitivo_svg():
    ...
    try:
        ...
        return jsonify(ok=certified,engine='Motor Polifan Definitivo V1.3',**result),(200 if certified else 422)
    except Exception as exc:
        return jsonify(ok=False,error=str(exc),engine='Motor Polifan Definitivo V1.3'),500
