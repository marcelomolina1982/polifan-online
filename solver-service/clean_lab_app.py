...
Objetivos:
- placa fisica nueva 1260 x 600 mm
- area util intencional del motor 1230 x 580 mm
- GAP duro de 3 mm
...
BUILD = "best-effort-multipass-v1-1230-lab-2026-08-27"
...
def _candidate_score(base,candidate,result):
    rows=base+[candidate]
    return (_material_area(rows),-float(candidate.get("priority") or 999999),-float(result.get("stripWidthMm") or 1e18))
...
def _best_same_set(current_result,challenger):
    ...
    if b<a-.25:return challenger
    if abs(b-a)<=.25 and float(challenger.get("solverDensity") or 0)>float(current_result.get("solverDensity") or 0):return challenger
    return current_result

@app.get("/health")
def health():
    return jsonify(ok=True,build=BUILD,mode="best-effort-multipass",solver=_identity(),widthCm=123,heightCm=58,gapMm=GAP_MM,minimumCompleteFigures=None,minimumDensity=None)

@app.get("/runtime-info")
def runtime_info():
    return jsonify(ok=True,build=BUILD,mode="best-effort-multipass",solver=_identity(),strategy=["anclar pedidos urgentes","rellenar iterativamente con futuros","probar candidatos por urgencia/area/compacidad","refinar con rotacion continua","devolver mejor resultado valido al vencer el tiempo"],widthCm=123,heightCm=58,gapMm=GAP_MM,minimumCompleteFigures=None,minimumDensity=None,defaultBudgetSeconds=DEFAULT_BUDGET_SECONDS)
...
def solve():
    ...
    attempts=[]; selected=None; best_result=None; anchor_count_used=0
    start_anchor=min(len(kits),max(1,requested_anchors))
    for count in range(start_anchor,0,-1):
        if time.time()-started>=budget-12:break
        rows=kits[:count]; seed=1103+count*97; seconds=min(QUICK_SECONDS+1,max(3,int(budget-(time.time()-started)-10)))
        result=core._run_sparrow(rows,GAP_MM,seconds,seed,continuous=False); _attempt(attempts,"urgent-anchor",f"top-{count}-urgentes",rows,result,seed,False)
    ...
    while time.time()-started<budget-20:
        ...
        for idx,cand in enumerate(candidates):
            ...
            rows=selected+[cand];seed=2309+fill_pass*401+idx*83+len(rows)*17;seconds=min(QUICK_SECONDS,max(3,int(remaining-17)))
            result=core._run_sparrow(rows,GAP_MM,seconds,seed,continuous=False);_attempt(attempts,"fill",f"agregar:{cand.get('figure')}",rows,result,seed,False)
    ...
    if time.time()-started<budget-16:
        for idx,cand in enumerate(_extension_candidates(selected,kits,limit=3)):
            remaining=budget-(time.time()-started)
            if remaining<14:break
            rows=selected+[cand];seed=4703+idx*131+len(rows)*19;seconds=min(8,max(4,int(remaining-10)))
            result=core._run_sparrow(rows,GAP_MM,seconds,seed,continuous=True);_attempt(attempts,"continuous-extra",f"rotacion-libre:{cand.get('figure')}",rows,result,seed,True)
