...
class RealUserGeometryStressTests(unittest.TestCase):
    ...
    def raw_payload(self, count, offset, reverse=False):
        rows=[]
        # Los primeros 10 son exactamente los 10 kits distintos recuperados del SVG real.
        # Para 11/12 se agregan duplicados reales, igual que puede ocurrir en producción.
    ...
    def certify_success(self, data, status):
    ...
    def test_fixture_is_traceable_to_real_uploaded_svg(self):
        ...
        self.assertEqual(len(source['kits']), 10)
        self.assertTrue(all(len(k['parts']) == 2 for k in source['kits']))
        # La reducción a 16 puntos es sólo para CI, pero debe mantener razonablemente
        # el área del contorno extraído del SVG real.
        for kit in source['kits']:
            for part in kit['parts']:
                poly=Polygon(part['points'])
                ref=float(part['area'])
                err=abs(float(poly.area)-ref)/max(1.0,ref)
                self.assertLess(err, .15, (kit['kit'], err))

    def test_stress_real_geometry_10_11_12_multiple_orders(self):
        results=[]
        # 12 ejecuciones completas del pipeline real. No abortamos al primer fallo:
        # medimos toda la matriz para conocer la confiabilidad real.
        for count in (10,11,12):
            for offset in range(4):
                ...
                if ok:
                    self.certify_success(data,status)
                    cert=data.get('productionCertificate') or {}
                    row={
                        'requested':count,'offset':offset,'reverse':reverse,'ok':True,
                    ...
                    row={
                        'requested':count,'offset':offset,'reverse':reverse,'ok':False,
                        'complete':0,'gap':0.0,'occupancy':0.0,'strip':0.0,
        ...
        success_by_count=Counter(r['requested'] for r in successful)
        full_target=Counter(r['requested'] for r in successful if r['complete']>=r['requested'])
        worst=max(r['elapsed'] for r in results)
        mean=sum(r['elapsed'] for r in results)/len(results)
        min_gap=min((r['gap'] for r in successful),default=0.0)
        summary={
            'source_sha256': self.fixture['source']['sha256'],
            'runs':len(results),
            'certified_runs':len(successful),
            'failed_runs':len(results)-len(successful),
            'mean_seconds':round(mean,2),
            'worst_seconds':round(worst,2),
            'min_certified_gap_mm':round(min_gap,4),
            'certified_by_requested':{str(k):success_by_count[k] for k in (10,11,12)},
            'full_target_successes':{str(k):full_target[k] for k in (10,11,12)},
            'results':results,
        }
        print('REAL_USER_STRESS_SUMMARY='+json.dumps(summary,separators=(',',':')))

        # Criterios duros para considerar al motor candidato a laboratorio visible.
        # Si fallan, queremos que CI quede rojo: no maquillamos el resultado.
        self.assertGreaterEqual(success_by_count[10],3,summary)
        self.assertGreaterEqual(len(successful),8,summary)
        self.assertGreaterEqual(min_gap,3.0,summary)
        self.assertLess(worst,45.0,summary)
