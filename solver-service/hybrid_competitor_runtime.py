...
def _group_score(group):
    ...
    # Preferimos densidad alta, figuras compactas y mezcla de formas largas/cortas.
    ...
    target_bonus=-abs(82.0-density)
    return (target_bonus, density, avg_comp, avg_sol, spread, -priority_pen)
...
def _seed_groups(pool,target,rng):
    ...
    seeds=[by_compact[:target],by_area[:target],by_small[:target],by_priority[:target]]
...
def _mutations(group,pool,rng):
    ...
    # Swaps de 1 figura.
    for idx in rng.sample(range(len(group)),min(4,len(group))):
        for new in rng.sample(remain,min(3,len(remain))):
            g=list(group);g[idx]=new;out.append(g)
    # Algunos swaps dobles para escapar de máximos locales.
...
def _run_packingsolver(selected,gap_mm,seconds):
    ...
    try:
        ...
        solution=Solver().solve(builder.build(),time_limit=max(4,int(seconds)),verbosity_level=0,optimization_mode='Anytime',
        ...
        return {'ok':True,'fits':xmax<=PLATE_W+0.5,'placements':placements,'density':density,'stripWidthMm':xmax,
                'continuousRotation':False,'source':'packingsolver-beam'},None
...
def hybrid_competition():
    ...
    try:
        ...
        if best:
            maybe12=_search_target(kits,validator,gap,12,started,diagnostics)
        ...
        if not best:
            out=dict(payload);out.update({'hybridCompetition':True,'hybridWinner':'Sparrow','hybridRuntimeVersion':RUNTIME_VERSION,
                'hybridStatus':f'V3 beam search probó {len(diagnostics)} combinaciones; fallback Sparrow 10','hybridDiagnostics':diagnostics,
                'selectionStrategy':f'HÍBRIDO V3 · {len(diagnostics)} combinaciones evolutivas · fallback Sparrow 10'})
            return jsonify(out)
        selected,result,certificate=best;target=len(selected);out=dict(payload)
        out.update({'engine':'Híbrido V3 beam search + PackingSolver + Certificador V1.7','completeFigures':target,
            ...
            'selectionStrategy':f'HÍBRIDO V3 · GANÓ beam search: {target} completas certificadas','targetDensityReached':float(result.get('density') or 0)>=80.0,
            'minimumGapMm':certificate.get('minimumGapMmCertified'),'requiredGapMm':MIN_GAP,'productionCertificate':certificate,
            'hybridCompetition':True,'hybridWinner':'PackingSolver-beam','hybridRuntimeVersion':RUNTIME_VERSION,
            'hybridStatus':f'V3 encontró {target} completas cambiando la composición del grupo durante la búsqueda','hybridDiagnostics':diagnostics,
