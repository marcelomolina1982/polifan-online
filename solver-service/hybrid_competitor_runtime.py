...
PLATE_H = 580.0
MIN_GAP = 3.0
MAX_COMPETITOR_SECONDS = 138.0
POOL_SIZE = 64
BEAM_WIDTH = 20
GENERATIONS = 8
MAX_SOLVER_TRIALS = 16
# PackingSolver soporta rotación continua nativa. No discretizamos más el espacio angular.
FREE_ROTATIONS = [(0.0, 360.0)]
RUNTIME_VERSION = 'hybrid-4.0-continuous-geometry-first'
...
def _metrics(k):
    ...
    for p in parts:
        try:
            ...
            ratios.append(max(w,h)/max(1.0,min(w,h))); widths.append(w); heights.append(h)
        except Exception: pass
    elong=sum(ratios)/len(ratios) if ratios else 1.0
    span=max(widths+heights) if widths or heights else 0.0
    return area,env,sol,elong,span


def _group_score(group):
    areas=[]; solids=[]; elongs=[]; compact=[]; spans=[]
    for k in group:
        a,e,s,l,sp=_metrics(k); areas.append(a); solids.append(s); elongs.append(l); compact.append(a/max(1.0,e)); spans.append(sp)
    ...
    span_spread=(max(spans)-min(spans)) if spans else 0
    priority_pen=sum(float(k.get('priority') or 999999) for k in group)/len(group)
    # Geometry-first: densidad + complementariedad de escalas/formas, luego urgencia.
    return (min(density,94.0), avg_comp, spread, span_spread, avg_sol, -priority_pen)
...
def _seed_groups(pool,target,rng):
    ...
    # Mezcla explícita grandes/chicas para crear complementariedad de huecos.
    mixed=[]
    lo=sorted(pool,key=lambda k:_metrics(k)[0]); hi=list(reversed(lo)); used=set()
    for a,b in zip(hi,lo):
        for k in (a,b):
            kid=str(k.get('kitId') or '')
            if kid not in used: used.add(kid); mixed.append(k)
            if len(mixed)>=target: break
        if len(mixed)>=target: break
    seeds=[by_compact[:target],by_area[:target],by_small[:target],by_priority[:target],by_elong[:target],mixed[:target]]
    top=pool[:min(len(pool),48)]
    for _ in range(18):
...
def _mutations(group,pool,rng):
    ...
    for idx in rng.sample(range(len(group)),min(8,len(group))):
        for new in rng.sample(remain,min(6,len(remain))):
    ...
    if len(group)>=3 and len(remain)>=3:
        for _ in range(7):
            ids=rng.sample(range(len(group)),3); news=rng.sample(remain,3); g=list(group)
...
def _beam_groups(kits,target=11):
    # No limitar por prioridad pura: la geometría necesita ver variedad suficiente.
    pool=sorted(kits,key=lambda k:(float(k.get('priority') or 999999),-_metrics(k)[0]))[:POOL_SIZE]
    rng=random.Random(271828+target)
    ...
    try:
        ...
        for kit in selected:
            for part in kit.get('parts') or []:
                returned=builder.add_item_type(part['geom'],copies=1,allowed_rotations=FREE_ROTATIONS)
                tid=item_type_id if returned is None else int(returned);part_map[tid]=part;item_type_id+=1
        solution=Solver().solve(builder.build(),time_limit=max(8,int(seconds)),verbosity_level=0,optimization_mode='Anytime',
            ...
            initial_maximum_approximation_ratio=0.18,maximum_approximation_ratio_factor=0.72,
            sequential_value_correction_subproblem_queue_size=128,not_anytime_tree_search_queue_size=512)
        ...
        for item in items:
            ...
            try:
                for shape in item.shapes: xmax=max(xmax,float(shape.bounds[2]))
            except Exception:pass
        ...
        return {'ok':True,'fits':xmax<=PLATE_W+0.5,'placements':placements,'density':density,'stripWidthMm':xmax,
                'continuousRotation':True,'source':'packingsolver-continuous-v4'},None
...
def _search_target(kits,validator,gap,target,started,diagnostics,deadline_seconds):
    ...
    for idx,g in enumerate(groups):
        ...
        per=max(8,min(14,int(remaining/trials_left)+2))
        ...
        row={'target':target,'trial':idx+1,'score':_group_score(g),'figures':[str(k.get('figure') or '') for k in g],
             'ok':bool(result and result.get('ok')),'fits':bool(result and result.get('fits')),'error':error,'solverSeconds':per,'rotation':'continua'}
...
def hybrid_competition():
    ...
    try:
        ...
        best=_search_target(kits,validator,gap,11,started,diagnostics,102.0)
        if best:
        ...
        if not best:
            out=dict(payload);out.update({'hybridCompetition':True,'hybridWinner':'Sparrow','hybridRuntimeVersion':RUNTIME_VERSION,
                'hybridStatus':f'V4 continua probó {len(diagnostics)} combinaciones 11+; fallback Sparrow 10','hybridDiagnostics':diagnostics,
                'selectionStrategy':f'HÍBRIDO V4 · rotación continua · pool recibido {len(kits)} · {len(diagnostics)} intentos · fallback Sparrow 10'})
            return jsonify(out)
        selected,result,certificate=best;target=len(selected);out=dict(payload)
        out.update({'engine':'Híbrido V4 geometry-first + PackingSolver continuo + Certificador V1.7','completeFigures':target,
            ...
            'selectionStrategy':f'HÍBRIDO V4 · GANÓ continuo: {target} completas certificadas','targetDensityReached':float(result.get('density') or 0)>=80.0,
            'minimumGapMm':certificate.get('minimumGapMmCertified'),'requiredGapMm':MIN_GAP,'productionCertificate':certificate,
            'hybridCompetition':True,'hybridWinner':'PackingSolver-continuous','hybridRuntimeVersion':RUNTIME_VERSION,
            'hybridStatus':f'V4 encontró {target} completas con rotación continua 0–360°','hybridDiagnostics':diagnostics,
            'bestSolutionPreserved':True})
        return jsonify(out)
    except Exception as exc:
        out=dict(payload);out.update({'hybridCompetition':True,'hybridWinner':'Sparrow','hybridRuntimeVersion':RUNTIME_VERSION,
            'hybridStatus':'V4 falló; fallback Sparrow','hybridDiagnosticError':str(exc)[:220],
            'selectionStrategy':'HÍBRIDO V4 · ERROR · fallback Sparrow 10'})
        return jsonify(out)
