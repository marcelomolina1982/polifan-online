...
# Runtime activo: presupuesto real + rescate con rotacion libre, integrado sobre la punta actual.
RUNTIME_REVISION='real-budget-2026-08-11'
MAX_INPUT_POOL=64
...
def adaptive_nest_sparrow():
    ...
    if not beam:return jsonify(ok=False,error='No se encontro una base inicial de 4.',attempts=attempts,runtimeRevision=RUNTIME_REVISION),422
    ...
    payload=response.get_json(); payload.update({'engine':'Sparrow incremental presupuesto real + huecos + V1.7','runtimeRevision':RUNTIME_REVISION,'baseProtected':True,'incrementalBase':True,'realSolverBudget':True,'baseAttempts':len(attempts),'candidatePool':len(kits),'inputKitsReceived':len(data.get('kits') or []),'minimumGapMm':gap})
