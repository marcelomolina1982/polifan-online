...
def run_level(kits, target, deadline, seconds_per_run=8.0, max_portfolios=14, max_workers=4, phase='race', stop_on_first=False):
    """Race diverse portfolios in small waves until the level deadline."""
    ...
    for start in range(0,len(jobs),wave_size):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for fut in as_completed(future_map):
                try:
                    ...
                    attempts.append({'phase':phase,'label':c.label,'seed':seed,'target':target,'certified':False,'error':str(exc)[:180]})
                    continue
        ...
        if best is not None and (stop_on_first or target == 10):
            break
        if best is not None:
            r=best['result'] or {}
...
def _optimistic_area_possible(kits, target):
    """Necessary-ish cheap filter only; never rejects borderline real cases.

    If even the target smallest complete kits consume virtually the whole plate,
    probing that count is usually wasted. 98% keeps this deliberately permissive.
    """
    if len(kits) < target:
        return False
...
def revolutionary_solve(prepared_kits, total_seconds=150.0, max_workers=4):
    """Safe base-10 + high-count probe + focused rescue.

    V2 spent too much of its budget walking 10 -> 11 -> 12. V2.1 first secures
    a certified 10, then probes the highest feasible counts from MAX downwards.
    If none succeeds, the remaining budget is concentrated on the recurrent
    real-world bottleneck at 11. A failed experiment can never destroy base-10.
    """
    started=time.time()
    ...
    # 1) Safety floor: secure one certified 10 quickly.
    base_seconds=max(12.0,min(24.0,budget*0.20))
    base_deadline=min(global_deadline,time.time()+base_seconds)
    base,attempts=run_level(
        prepared_kits,10,deadline=base_deadline,
        seconds_per_run=5.5,max_portfolios=14,max_workers=max_workers,
        phase='base10',stop_on_first=True,
    )
    all_attempts.extend(attempts)

    # One stronger base retry if the fast race failed.
    if base is None and global_deadline-time.time() >= 8.0:
        ...
        base,attempts=run_level(
            prepared_kits,10,deadline=retry_deadline,
            seconds_per_run=7.5,max_portfolios=24,max_workers=max_workers,
            phase='base10-rescue',stop_on_first=True,
        )
        all_attempts.extend(attempts)
    ...
    # 2) Revolutionary count probe: test highest counts first with short races.
    # The first certified level found is already the highest count reached in
    # this descending search, so lower counts cannot beat it lexicographically.
    high_success=None
    for target in range(last_target,10,-1):
        remaining=global_deadline-time.time()
        if remaining < 8.0:
            break
        if not _optimistic_area_possible(prepared_kits,target):
            all_attempts.append({'phase':'high-probe-skip','target':target,'reason':'area bound'})
            continue

        # Preserve a meaningful reserve for target-11 rescue if high probes fail.
        reserve11=min(32.0,max(14.0,budget*0.24))
        usable=max(0.0,remaining-reserve11) if target>11 else remaining
        if target>11 and usable < 6.0:
            continue
        probe_seconds=min(14.0 if target>=14 else 17.0, max(6.0,usable))
        probe_deadline=min(global_deadline,time.time()+probe_seconds)
        level,attempts=run_level(
            prepared_kits,target,deadline=probe_deadline,
            seconds_per_run=4.5 if target>=14 else 5.5,
            max_portfolios=10 if target>=14 else 14,
            max_workers=max_workers,phase='high-probe',stop_on_first=True,
        )
        all_attempts.extend(attempts)
    ...
    # 3) If a high count succeeded, spend only a modest remainder refining that
    # same count. Do not waste time on lower counts, because count dominates.
    if high_success is not None:
        target=len(high_success['candidate'].kits)
        remaining=global_deadline-time.time()
        if remaining >= 8.0:
            refine_deadline=min(global_deadline,time.time()+min(22.0,remaining))
            refined,attempts=run_level(
                prepared_kits,target,deadline=refine_deadline,
                seconds_per_run=6.5,max_portfolios=18,max_workers=max_workers,
                phase='high-refine',stop_on_first=False,
            )
            all_attempts.extend(attempts)
            if refined is not None and solution_score(refined['candidate'].kits,refined['result']) > solution_score(best['candidate'].kits,best['result']):
                best=refined
    else:
        # 4) No high target worked: concentrate the rest on the real bottleneck 11.
        remaining=global_deadline-time.time()
        if last_target>=11 and remaining >= 6.0:
            rescue_deadline=global_deadline
            level,attempts=run_level(
                prepared_kits,11,deadline=rescue_deadline,
                seconds_per_run=8.0,max_portfolios=32,max_workers=max_workers,
                phase='target11-rescue',stop_on_first=False,
            )
            all_attempts.extend(attempts)
            if level is not None and solution_score(level['candidate'].kits,level['result']) > solution_score(best['candidate'].kits,best['result']):
                best=level
