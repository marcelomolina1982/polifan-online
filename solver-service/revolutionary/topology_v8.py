...
Manual layouts are used only as spatial seeds and are independently certified.
V9 also recognizes source pieces stored rotated 90 degrees in historical fixtures.
"""
...
def _mama_like_dims(dims):
    w,h=dims
    return (265<=w<=295 and 82<=h<=108) or (265<=h<=295 and 82<=w<=108)

def _rotation_for_slot(part,sw,sh):
    w,h=_dims(part)
    direct=abs(w-sw)+abs(h-sh);swapped=abs(h-sw)+abs(w-sh)
    return 90.0 if swapped+1e-6<direct else 0.0

def _placement(part,slot):
    sx,sy,sw,sh=slot;angle=_rotation_for_slot(part,sw,sh)
...
def mama_seed(prepared_kits):
    ...
    for k in prepared_kits:
        ...
        if len(parts)==2 and all(_mama_like_dims(_dims(p)) for p in parts):candidates.append(k)
    if len(candidates)<12:return None
    kits=candidates[:12];placements=[]
    for i,k in enumerate(kits):
        parts=list(k['parts'])
        direct=_orientation_distance(_dims(parts[0]),MAMA_EXPECTED[0])+_orientation_distance(_dims(parts[1]),MAMA_EXPECTED[1])
        swapped=_orientation_distance(_dims(parts[1]),MAMA_EXPECTED[0])+_orientation_distance(_dims(parts[0]),MAMA_EXPECTED[1])
        pa,pb=(parts[0],parts[1]) if direct<=swapped else (parts[1],parts[0])
...
def ball_rose_seed(prepared_kits):
    ...
    ball_parts=[p for k in balls[:5] for p in k['parts']];rose_parts=[p for k in roses[:5] for p in k['parts']]
