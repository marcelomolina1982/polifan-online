"""V9 truth benchmark using real geometry already stored in the historical plate fixture."""
from __future__ import annotations
import base64,copy,gzip,json,threading,time
...
def _prepared_from_payload(payload):
    ...
    for order,(kid,rows) in enumerate(grouped.items()):
        ...
        for idx,row in enumerate(rows):
            ...
            minx,miny,maxx,maxy=g.bounds;area=float(g.area);env=max(1.0,float((maxx-minx)*(maxy-miny)));role=str(row.get('role') or ('base' if idx==0 else 'tapa'))
...
def _mama_distance(kit):
    ...
    if not all(((260<=w<=300 and 78<=h<=112) or (260<=h<=300 and 78<=w<=112)) for w,h in dims):return 1e9
    e=[(280.212,95.189),(280.001,94.472)]
    direct=_odist(dims[0],e[0])+_odist(dims[1],e[1]);swapped=_odist(dims[1],e[0])+_odist(dims[0],e[1])
    return min(direct,swapped)

def _clone_complete_kit(source,index):
    k=copy.deepcopy(source);kid=f'pure-mama-{index:02d}';k['kitId']=kid;k['figure']='Palabra Mama Imprenta';k['priority']=1.0;k['date']='2026-08-22'
    for pidx,p in enumerate(k['parts']):p['kitId']=kid;p['figure']='Palabra Mama Imprenta';p['instanceId']=f'{kid}-p{pidx}'
    return k

def pure_mama_prepared(total=16):
    historical=_prepared_from_payload(_load_payload('plate06_mama_case.gz.b64'));ranked=sorted(historical,key=_mama_distance)
    if not ranked or _mama_distance(ranked[0])>=1e8:
        dims=[[(round(_dims(p)[0],2),round(_dims(p)[1],2)) for p in k.get('parts') or []] for k in historical];raise RuntimeError(f'No Mamá-like complete kit found. dims={dims[:20]}')
    source=ranked[0];return [_clone_complete_kit(source,i+1) for i in range(int(total))],source

def topology_probe():
    kits,source=pure_mama_prepared(16);seeds=workshop_seeds(kits);rows=[]
    for seed in seeds:
...
def run_pure_mama(seconds=95.0):
    kits,source=pure_mama_prepared(16);started=time.time();r=revolutionary_solve_v8(kits,total_seconds=float(seconds),max_workers=4);count=int(r.get('completeFigures') or 0);gap=float(r.get('minimumGapMm') or 0)
