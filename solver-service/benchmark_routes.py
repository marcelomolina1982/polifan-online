...
def _extract_piece_kits(svg_text):
    ...
    return _extract_legacy_kits(svg_text,root),'historical-connected-components'

def _metrics(kits,result):
    area=sum(float(k.get('area') or 0) for k in kits)
...
def _placed_geometries(kits,placements):
    ...
    for p in placements:
        ...
        g=shp_translate(g,xoff=float(p.get('xCm') or 0)*10.0,yoff=float(p.get('yCm') or 0)*10.0)
        rows.append((p,g))
    return rows
...
def _validate_layout(kits,placements):
    ...
    for p,g in rows:
        ...
        if minx < -tol or miny < -tol or maxx > PLATE_WIDTH_MM+tol or maxy > PLATE_HEIGHT_MM+tol:
    ...
    for i in range(len(rows)):
        for j in range(i+1,len(rows)):
            ...
            if d<ABSOLUTE_MIN_GAP_MM-0.001:
...
def _execute_svg(svg_text):
    ...
    if result is None:
        return {'ok':False,'error':'No se pudo reacomodar el conjunto completo','traceId':trace_id,'pieceCount':len(kits),'parser':parser,'gapMm':GAP_MM,'workspaceMm':[PLATE_WIDTH_MM,PLATE_HEIGHT_MM],'attempts':attempts,'elapsedSeconds':elapsed},422
    placements=result.get('placements') or []
    validation,rows=_validate_layout(kits,placements); preview_url=None
    if rows:
        _BENCH_RESULTS[trace_id]=_svg_preview(rows); preview_url=f'/benchmark-result/{trace_id}.svg'
...
def upload_benchmark():
    if request.method=='GET':
        return '<!doctype html><html><body><h2>Prueba historica real · 1230×580 · 1:1</h2><form method="post" enctype="multipart/form-data"><input type="file" name="file" accept=".svg,image/svg+xml" required><button type="submit">Buscar y validar placa</button></form></body></html>'
    ...
    try:
        ...
        return jsonify(ok=False,error=f'No se pudo leer/procesar el SVG: {exc}'),422
...
def _private_raw_fixture_svg():
    return _joined_env('POLIFAN_BENCH_FIXTURE_SVG_PART')
...
def benchmark_private_fixture():
    ...
    if raw_svg:
        try:
            payload,status=_execute_svg(raw_svg); return jsonify(payload),status
        except Exception as exc:
            return jsonify(ok=False,error=f'Fixture SVG privado invalido: {exc}'),422
    packed=_private_fixture_payload()
    if not packed: return jsonify(ok=False,error='Fixture privado no configurado'),404
    try:
        svg_text=gzip.decompress(base64.b64decode(packed)).decode('utf-8-sig')
        payload,status=_execute_svg(svg_text); return jsonify(payload),status
    except Exception as exc:
        return jsonify(ok=False,error=f'Fixture privado invalido: {exc}'),422
