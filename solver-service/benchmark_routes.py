...
import re, time, uuid, os, base64, gzip
...
def _source_size_mm(root):
    ...
    if len(vb)==4 and vb[2]>0 and vb[3]>0: return float(vb[2]),float(vb[3])
    return PLATE_WIDTH_MM,PLATE_HEIGHT_MM

def _expand_uses(svg_text):
    root=ET.fromstring(svg_text)
    refs={str(el.attrib.get('id')):deepcopy(el) for el in root.iter() if el.tag.split('}')[-1]=='path' and el.attrib.get('id')}
    def walk(parent):
        children=list(parent)
        for pos,ch in enumerate(children):
            tag=ch.tag.split('}')[-1].lower()
            if tag=='use':
                href=ch.attrib.get(f'{{{XLINK_NS}}}href') or ch.attrib.get('href') or ''
                src=refs.get(href.lstrip('#'))
                if src is None: continue
                repl=deepcopy(src)
                repl.attrib.pop('id',None)
                x=str(ch.attrib.get('x') or '0'); y=str(ch.attrib.get('y') or '0')
                transforms=[f'translate({x},{y})']
                if ch.attrib.get('transform'): transforms.append(ch.attrib.get('transform'))
                if repl.attrib.get('transform'): transforms.append(repl.attrib.get('transform'))
                repl.set('transform',' '.join(transforms))
                for k,v in ch.attrib.items():
                    local=k.split('}')[-1]
                    if local not in ('href','x','y','transform'): repl.set(k,v)
                parent.remove(ch); parent.insert(pos,repl)
            elif tag!='defs':
                walk(ch)
        for ch in list(parent):
            if ch.tag.split('}')[-1].lower()=='defs': parent.remove(ch)
    walk(root)
    return ET.tostring(root,encoding='unicode')

def _kit_from_geom(geom,idx,prefix='legacy'):
    minx,miny,maxx,maxy=geom.bounds; geom=shp_translate(geom,xoff=-minx,yoff=-miny); kid=f'{prefix}_{idx:03d}'
...
def _extract_tagged_kits(svg_text,root):
    ...
    for g in root.iter():
        ...
        wrapper.append(piece); piece_svg=ET.tostring(wrapper,encoding='unicode')
...
def _extract_legacy_kits(svg_text,root):
    ...
    components=[g for g in components if not g.is_empty and g.area>0.2]; components.sort(key=lambda g:(round(g.bounds[1],3),round(g.bounds[0],3)))
    return [_kit_from_geom(g,i+1) for i,g in enumerate(components)]

def _extract_piece_kits(svg_text):
    expanded=_expand_uses(svg_text) if '<use' in svg_text else svg_text
    root=ET.fromstring(expanded); tagged=_extract_tagged_kits(expanded,root)
    if tagged: return tagged,'polifan-tagged'
    return _extract_legacy_kits(expanded,root),'historical-connected-components'
...
def _svg_preview(rows):
    ...
    for _,g in rows:
        for poly in _rings(g):
            ...
            if pts: chunks.append('<path d="M '+' L '.join(f'{x:.3f},{y:.3f}' for x,y in pts)+' Z" fill="none" stroke="black" stroke-width="0.7"/>')
    chunks.append('</svg>'); return ''.join(chunks)

def _execute_svg(svg_text):
    kits,parser=_extract_piece_kits(svg_text)
    ...
    if result is None: return {'ok':False,'error':'No se pudo reacomodar el conjunto completo','traceId':trace_id,'pieceCount':len(kits),'parser':parser,'gapMm':GAP_MM,'workspaceMm':[PLATE_WIDTH_MM,PLATE_HEIGHT_MM],'attempts':attempts,'elapsedSeconds':elapsed},422
    ...
    return {'ok':bool(validation.get('ok')),'engine':'Sparrow historical-SVG compatibility + hard validation','traceId':trace_id,'pieceCount':len(kits),'parser':parser,'placements':placements,'gapMm':GAP_MM,'widthCm':PLATE_WIDTH_MM/10.0,'heightCm':PLATE_HEIGHT_MM/10.0,'attempts':attempts,'elapsedSeconds':elapsed,'layoutValidation':validation,'previewSvgUrl':preview_url,**_metrics(kits,result)},200
...
def upload_benchmark():
    if request.method=='GET': return '<!doctype html><html><body><h2>Prueba historica real · 1230×580 · 1:1</h2><form method="post" enctype="multipart/form-data"><input type="file" name="file" accept=".svg,image/svg+xml" required><button type="submit">Buscar y validar placa</button></form></body></html>'
...
def _joined_env(prefix):
    ...
    for idx in range(1,100):
        part=str(os.environ.get(f'{prefix}_{idx}') or '')
        if not part: break
        parts.append(part)
    return ''.join(parts)
...
def _private_fixture_payload():
    ...
    return packed or _joined_env('POLIFAN_BENCH_FIXTURE_GZIP_B64_PART')

def _private_raw_fixture_svg(): return _joined_env('POLIFAN_BENCH_FIXTURE_SVG_PART')

@app.get('/benchmark-private-fixture')
def benchmark_private_fixture():
    raw_svg=_private_raw_fixture_svg()
    if raw_svg:
        try: payload,status=_execute_svg(raw_svg); return jsonify(payload),status
        except Exception as exc: return jsonify(ok=False,error=f'Fixture SVG privado invalido: {exc}'),422
    ...
    except Exception as exc: return jsonify(ok=False,error=f'Fixture privado invalido: {exc}'),422
