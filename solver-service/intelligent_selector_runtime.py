...
# Sparrow Smart V1.9: conserva una base certificada de 10 y reparte el tiempo
# de crecimiento entre TODOS los niveles 11..16. Ningún nivel puede consumir
# el presupuesto completo e impedir que se prueben los siguientes.
MAX_POOL=64
...
def intelligent_nest():
    ...
    if base is None:
        for idx,(label,g) in enumerate(groups):
            ...
            if valid:base=(g,r,cert);break
            _learn_failed(g,r)
    if base is None:return jsonify(ok=False,error='No se encontró una base de 10 certificada.',engine='Smart V1.9 Balanced Growth',selectorVersion='smart-v19-balanced',attempts=attempts),422

    best_selected,best_result,best_cert=base
    for x in best_selected:_memory[_key(x)]=max(-1.5,_memory.get(_key(x),0.0)-.25)

    # Balanced Growth: cada nivel recibe su propia ventana. Así 11 no puede
    # comerse los 180 s. Se prueban 11,12,13,14,15,16 obligatoriamente mientras
    # quede tiempo global. La mejor placa certificada siempre gana y base 10 queda segura.
    levels_tried=[]
    for target in range(11,min(MAX_GROWTH_TARGET,len(kits))+1):
        ...
        candidates=_priority_safe_candidates(kits,target,GROWTH_CANDIDATES)
    ...
    response=ns._result_payload(best_selected,f'Smart V1.9 Balanced Growth: {len(best_selected)} completas · objetivo 70%',best_result,kits,rejected,attempts,started,None)
    payload=response.get_json();payload.update({'engine':'Sparrow Smart V1.9 Balanced Growth · base 10 + 11/12/13/14/15/16','selectorVersion':'smart-v19-balanced','smartSelection':True,'candidatePool':len(kits),'minimumGapMm':best_cert.get('minimumGapMmCertified'),'requiredGapMm':float(getattr(ns,'MIN_PRODUCTION_GAP_MM',LAB_GAP_MM)),'protectedBase10':True,'improvedAbove10':len(best_selected)>10,'completeFigures':len(best_selected),'productiveTargetPercent':PRODUCTIVE_TARGET_PERCENT,'productiveTargetReached':float(best_result.get('density') or 0)>=PRODUCTIVE_TARGET_PERCENT,'hardTotalLimitSeconds':TOTAL_SECONDS,'perGrowthLevelSeconds':PER_LEVEL_SECONDS,'labGapMm':LAB_GAP_MM,'levelsTried':levels_tried,'growthContinuesAfterMiss':True})
