...
def _direct_blockers(offenders,geoms,gap,w):
    ...
    for opid in offenders:
        ...
        for pid,g in geoms.items():
            ...
            if corridor.intersects(g): scores[pid]=scores.get(pid,0)+100000+corridor.intersection(g).area
            else:
                d=target.distance(g)
    ...
    fallback=[p for p in geoms if p not in offenders and p not in ranked]; fallback.sort(key=lambda p:v6._neighbor_rank(p,offenders,geoms,gap))
    return ranked+fallback,ranked

def _target_compact(kits,base_result,gap,w,h,seconds):
    parts=v4._part_map(kits); placements=base_result.get('placements') or []
    if len(placements)!=len(parts): return None
    poses={str(p.get('instanceId')):deepcopy(p) for p in placements}; geoms={p:v4._geom(parts[p],q) for p,q in poses.items()}
    deadline=time.time()+max(24,int(seconds)+6); best_width=max(g.bounds[2] for g in geoms.values()); best_poses=deepcopy(poses); stagnant=0
    for sweep in range(16):
        if time.time()>=deadline: break
        offenders=[p for p,g in geoms.items() if g.bounds[2]>w+.35]
        _,blockers=_direct_blockers(offenders,geoms,gap,w) if offenders else ([],[])
        right=sorted(geoms,key=lambda p:geoms[p].bounds[2],reverse=True); left=sorted(geoms,key=lambda p:geoms[p].bounds[0])
        # Clear the actual leftward corridors first, then move the overflowing pieces, then globally tighten.
        order=[]
        for p in blockers[:7]+offenders+right+left:
            if p not in order: order.append(p)
        changed=False
        for pid in order:
            if time.time()>=deadline: break
            found=v4._move_one(pid,parts,poses,geoms,gap,w,h,fine=(sweep>=1))
            if not found: continue
            pose,g=found; old_global=max(x.bounds[2] for x in geoms.values()); old_x=geoms[pid].bounds[0]
            new_global=max([g.bounds[2]]+[og.bounds[2] for oid,og in geoms.items() if oid!=pid])
            # For corridor blockers allow a meaningful left move even before global width drops.
            is_blocker=pid in blockers
            if new_global<=old_global+.05 and (new_global<old_global-.02 or g.bounds[0]<old_x-(.15 if is_blocker else .05)):
                poses[pid].update(pose); geoms[pid]=g; changed=True
        width=max(g.bounds[2] for g in geoms.values())
        if width<best_width-.01: best_width=width; best_poses=deepcopy(poses); stagnant=0
        else: stagnant+=1
        if best_width<=w+.35: break
        if stagnant>=4 and sweep>=5: break
        if not changed and sweep>=3: break
    out=[]
    for p in placements:
        pid=str(p.get('instanceId')); q=deepcopy(p); q.update(best_poses[pid]); out.append(q)
    result=deepcopy(base_result); result['placements']=out; result['stripWidthMm']=float(best_width); result['fits']=best_width<=w+.5; result['repairApplied']=True
    ...
    result['repairStrategy']='target-blocker-global-v7'
    return result

def _bounded_insert_candidates(pid,parts,original_geoms,original_poses,occupied,gap,w,h,limit=10):
    deadline=time.time()+PER_CALL_SECONDS; part=parts[pid]; current=original_geoms[pid]; cminx,cminy,_,_=current.bounds; a0=float(original_poses[pid].get('angle') or 0)%360; obstacles=list(occupied.values())
    ...
    for angle in angles:
        ...
        xs=sorted(set(round(max(0,min(maxx,x)),3) for x in xs if math.isfinite(x)),key=lambda x:(abs(x-cminx),x))[:MAX_X]; ys=sorted(set(round(max(0,min(maxy,y)),3) for y in ys if math.isfinite(y)),key=lambda y:(abs(y-cminy),y))[:MAX_Y]
        hits=0
        for x in xs:
            if time.time()>=deadline or tested>=MAX_TESTED_POSES: break
            for y in ys:
                ...
                move=abs(g.bounds[0]-cminx)+abs(g.bounds[1]-cminy); found.append(((g.bounds[2],move,abs(g.bounds[1]-cminy),v6._angle_delta(angle,a0)),pose,g)); hits+=1
                if hits>=5: break
            if hits>=5: break
...
def _orders(subset,geoms,offenders,blockers):
    ...
    add(bs+os+rest); add(os+bs+rest); add(sorted(subset,key=lambda p:(geoms[p].area,-geoms[p].bounds[2])))
    return out
...
def _frontier_repack_v7(kits,base_result,gap,w,h,seconds):
    ...
    poses={str(p.get('instanceId')):deepcopy(p) for p in placements}; geoms={p:v4._geom(parts[p],q) for p,q in poses.items()}; offenders=[p for p,g in geoms.items() if g.bounds[2]>w+.35]
    if not offenders or len(offenders)>8: return None,{'reason':'bad-offender-count','offenders':len(offenders)}
    neighbors,direct=_direct_blockers(offenders,geoms,gap,w); deadline=time.time()+max(28,int(seconds)); best=None; best_width=float('inf'); tested=0; orders_tried=0; stages=0
    starts=[]; direct_count=min(4,len(direct))
        ...
        for order in _orders(subset,geoms,offenders,direct):
            ...
            for pid in order:
                ...
                for _,base_move,placed,occupied in beam[:10]:
                    if time.time()>=deadline: break
                    cand=_bounded_insert_candidates(pid,parts,geoms,poses,occupied,gap,w,h,limit=8); tested+=len(cand)
                    for score,pose,g in cand:
                        np=dict(placed); np[pid]=(pose,g); no=dict(occupied); no[pid]=g; width=max((x.bounds[2] for x in no.values()),default=0); move=base_move+abs(g.bounds[0]-geoms[pid].bounds[0])+abs(g.bounds[1]-geoms[pid].bounds[1]); nxt.append((width,move,np,no))
    ...
    for p in placements:
        ...
        out.append(q)
    result=deepcopy(base_result); result['placements']=out; result['stripWidthMm']=float(best_width); result['fits']=best_width<=w+.5; result['repairApplied']=True; result['repairMovedPieces']=moved; result['repairStrategy']='swept-blocker-frontier-v7'; diag.update({'reason':'success' if result['fits'] else 'best-still-wide','widthMm':round(best_width,3),'movedPieces':moved})
    return result,diag

# Inject both target-aware compaction and the bounded final repair into the isolated V7 benchmark only.
v4._global_compact=_target_compact
v6.v5.v4._global_compact=_target_compact
