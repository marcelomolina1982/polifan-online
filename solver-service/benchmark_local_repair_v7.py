...
PER_CALL_SECONDS=.55
MAX_TESTED_POSES=2800
MAX_X=54
MAX_Y=64

def _bounded_insert_candidates(pid,parts,original_geoms,original_poses,occupied,gap,w,h,limit=12):
    ...
    offsets=[0,-3,3,-5,5,-8,8,-10,10,-15,15,-22.5,22.5,-30,30,-45,45,-90,90,180]
    angles=[]
    for off in offsets:
    ...
    for angle in angles:
        ...
        for step in (2.5,5,10):
            for k in range(22): xs.append(cminx-k*step)
            for k in range(-12,13): ys.append(cminy+k*step)
        for og in obstacles:
...
def _orders(subset,geoms,offenders):
    out=[]
    def add(s):
        t=tuple(s)
        if t not in out: out.append(t)
    add(sorted(subset,key=lambda p:(-geoms[p].bounds[2],geoms[p].area)))
    add(sorted(subset,key=lambda p:(geoms[p].area,-geoms[p].bounds[2])))
    add([p for p in subset if p not in offenders]+[p for p in offenders if p in subset])
    add([p for p in offenders if p in subset]+[p for p in subset if p not in offenders])
    return out
...
def _frontier_repack_v7(kits,base_result,gap,w,h,seconds):
    ...
    neighbors=[p for p in geoms if p not in offenders]; neighbors.sort(key=lambda p:v6._neighbor_rank(p,offenders,geoms,gap))
    deadline=time.time()+max(28,int(seconds)); best=None; best_width=float('inf'); tested=0; orders_tried=0; stages=0
    # Progressive neighborhoods: solve the smallest useful local problem first; expand only when needed.
    max_extra=min(6,len(neighbors))
    for extra in range(1,max_extra+1):
        if time.time()>=deadline: break
        subset=tuple(offenders+neighbors[:extra]); stages+=1; evac=set(subset); fixed={p:g for p,g in geoms.items() if p not in evac}
        if any(g.bounds[2]>w+.35 for g in fixed.values()): continue
        for order in _orders(subset,geoms,offenders):
            if time.time()>=deadline: break
            orders_tried+=1; beam=[(max((g.bounds[2] for g in fixed.values()),default=0),0,{},fixed)]
            for pid in order:
                nxt=[]
                for _,base_move,placed,occupied in beam[:16]:
                    if time.time()>=deadline: break
                    for score,pose,g in _bounded_insert_candidates(pid,parts,geoms,poses,occupied,gap,w,h,limit=10):
                        tested+=1; np=dict(placed); np[pid]=(pose,g); no=dict(occupied); no[pid]=g
                        ...
                        # Keep partial states that actually pull the frontier left; movement is secondary.
                        nxt.append((width,move,np,no))
                nxt.sort(key=lambda r:(r[0],r[1])); beam=nxt[:16]
                if not beam: break
        ...
        if best_width<=w+.35: break
    if best is None:
        return None,{'reason':'staged-no-solution','offenders':len(offenders),'stagesTried':stages,'ordersTried':orders_tried,'testedCandidates':tested}
    out=[]; moved=0
    for p in placements:
        q=deepcopy(p); pid=str(p.get('instanceId'))
        if pid in best: q.update(best[pid][0]); moved+=1
        out.append(q)
    result=deepcopy(base_result); result['placements']=out; result['stripWidthMm']=float(best_width); result['fits']=best_width<=w+.5; result['repairApplied']=True; result['repairMovedPieces']=moved; result['repairStrategy']='staged-local-frontier-v7'
    return result,{'reason':'success' if result['fits'] else 'best-still-wide','widthMm':round(best_width,3),'movedPieces':moved,'stagesTried':stages,'ordersTried':orders_tried,'testedCandidates':tested}
