...
import OrdersFinance from './pages/OrdersFinance'
...
export default function App(){
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({data})=>{setSession(data.session);if(data.session) loadData(!cachedState);else setLoading(false)})
    const {data:{subscription}} = supabase.auth.onAuthStateChange((_,s)=>{setSession(s);if(s) loadData(false)})
  ...
  async function loadData(initial=false,silent=false){
    ...
    if(error){if(!silent) alert('No se pudo conectar con Supabase: '+error.message);setLoading(false);return}
    const next=data?.data ? {...emptyState(),...data.data} : emptyState();setDb(next);try{localStorage.setItem('polifan-app-cache',JSON.stringify(next))}catch{};setLoading(false)
  }

  async function saveData(next){
    setSaving(true);const previous=db,updatedAt=new Date().toISOString()
    ...
    setSaving(false)
    if(error){setDb(previous);alert('No se pudo guardar en Supabase. El cambio no fue aplicado: '+error.message);return {ok:false,error}}
    const confirmed=saved?.data ? {...emptyState(),...saved.data} : next;setDb(confirmed);try{localStorage.setItem('polifan-app-cache',JSON.stringify(confirmed))}catch{};return {ok:true,updatedAt:saved?.updated_at||updatedAt}
  }
  ...
  return <div className="app">
    ...
    <div className="content">
      <header><button className="menu" onClick={()=>setMobileOpen(v=>!v)}>☰</button><div className="header-right"><span className={'sync '+(saving?'saving':'')}>{saving?'Guardando…':'Guardado online'}</span><div className="avatar">{session.user.email?.[0]?.toUpperCase()||'A'}</div><div className="user"><b>{session.user.email?.split('@')[0]}</b><small>Administrador</small></div><button className="ghost" onClick={logout}>Salir</button></div></header>
      <main>
        ...
        {page==='orders' && <><Orders db={db} onSave={saveData} onEdit={(o)=>{setEditingOrder(o);setPage('new')}}/><OrdersFinance db={db}/></>}
