...
export default function Clients({db,onSave}){
  ...
  const blank={firstName:'',lastName:'',name:'',phone:'',dni:'',address:'',locality:'',district:'',province:'',postalCode:'',notes:''}
  ...
  async function add(e){
    ...
    const next=upsertClient(clients,{...form,name:fullName})
  ...
  const list=clients.filter(c=>`${c.name} ${c.phone} ${c.dni||''} ${c.address||''} ${c.locality||c.zone||''} ${c.district||''} ${c.province||''} ${c.postalCode||''}`.toLowerCase().includes(q.toLowerCase()))
  ...
  return <>
    ...
    <form className="panel" onSubmit={add}><div className="form-grid">
      <Field label="Nombre"><input value={form.firstName} onChange={e=>setForm({...form,firstName:e.target.value,name:[e.target.value,form.lastName].filter(Boolean).join(' ')})}/></Field>
      <Field label="Apellido"><input value={form.lastName} onChange={e=>setForm({...form,lastName:e.target.value,name:[form.firstName,e.target.value].filter(Boolean).join(' ')})}/></Field>
      ...
      <Field label="Partido / Departamento"><input value={form.district} onChange={e=>setForm({...form,district:e.target.value})}/></Field>
