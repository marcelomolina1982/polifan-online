...
export default function CutList({db,goMotor}){
  ...
  const allGroups=useMemo(()=>pendingCutPlan(liveDb),[liveDb])
  ...
  const summaryRows=useMemo(()=>{
    ...
    const inv=Object.fromEntries(productionStockSnapshot(liveDb).map(r=>[normalizeFigureKey(r.figure),r]))
  ...
  function printDailyList(){
    ...
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Lista para cortar</title><style>@page{size:A4 portrait;margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#111;margin:0;font-size:12px}header{text-align:center;border-bottom:2px solid #111;padding-bottom:8px;margin-bottom:12px}h1{font-size:20px;margin:0 0 4px}header p{margin:0}section{break-inside:avoid;margin:0 0 14px}h2{font-size:15px;margin:0;background:#eee;padding:7px;border:1px solid #111}.orders{margin:5px 0;font-size:10px;color:#444}table{width:100%;border-collapse:collapse}th,td{border:1px solid #111;padding:6px;text-align:left}th:last-child,td:last-child{width:26%;text-align:center;font-weight:700}tfoot th{background:#f3f3f3}.note{margin-top:12px;font-size:10px;text-align:center}</style></head><body><header><h1>TU VIDA EN TINTA · POLIFAN</h1><p>LISTA DE PIEZAS PARA CORTAR</p></header>${sections}<p class="note">Cálculo único: pedidos activos + stock producido + piezas ya en corte. Los pedidos vencidos activos conservan su reserva hasta ser Entregados o Cancelados.</p><script>window.onload=()=>window.print()</script></body></html>`)
    win.document.close()
  }

  return <>
    <Title title="Pedidos para cortar" sub="Fuente operativa en vivo: pedidos activos, inventario producido y piezas ya en corte." actions={<div className="actions"><button className="primary" onClick={goMotor}>Generar placas</button><button className="ghost" onClick={printDailyList}>Imprimir lista</button></div>}/>
    <div className="notice"><b>{refreshing?'Actualizando producción…':'Producción sincronizada con Supabase'}</b><span>El cálculo conserva la reserva de todos los pedidos activos. Lo producido y lo que ya está En corte se descuentan del faltante para no cortar dos veces.</span></div>
    {overdueOrders>0&&<div className="notice" style={{borderColor:'#ef4444'}}><b>⚠ {overdueOrders} pedidos activos con entrega vencida</b><span>Siguen reservando sus piezas porque no existe una salida individual registrada. No se eliminan por fecha automáticamente.</span></div>}
