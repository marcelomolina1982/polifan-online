...
export default function Quotes({db,onSave,onOpenOrder}){
 ...
 return <>
  ...
  <div className="notice" style={{marginBottom:14}}><b>Flujo recomendado</b><span>Cuando el cliente confirme, usá “Abrir en Nuevo pedido” para revisar fecha, despacho y datos antes de guardarlo. Al guardar, el presupuesto pasa automáticamente a Aprobado.</span></div>
  <div className="request-tabs"><button className={view==='pending'?'active':''} onClick={()=>setView('pending')}>Pendientes ({quotes.filter(active).length})</button><button className={view==='history'?'active':''} onClick={()=>setView('history')}>Historial ({quotes.filter(q=>!active(q)).length})</button></div>
  <div className="panel table-wrap operational-table"><table><thead><tr><th>Presupuesto</th><th>Cliente</th><th>Origen</th><th>Piezas</th><th>Total</th><th>Estado</th><th className="actions-cell">Acciones</th></tr></thead><tbody>{visible.map(q=><tr key={q.id}><td data-label="Presupuesto"><b>{q.code}</b><small className="block">{q.date||''}</small>{q.orderNumber&&<small className="block green-text">Pedido #{q.orderNumber}</small>}</td><td><b>{q.client||q.customer?.name||'Sin nombre'}</b><small className="block">{q.phone||q.customer?.phone||''}</small></td><td data-label="Origen">{q.source||'Manual'}</td><td data-label="Piezas">{(q.items||[]).reduce((a,i)=>a+Number(i.qty||0),0)}</td><td data-label="Total"><b>{money(q.total)}</b></td><td data-label="Estado"><span className={`quote-status ${q.status==='Aprobado'?'approved':q.status==='Cancelado'?'cancelled':'pending'}`}>{q.status||'Pendiente'}</span></td><td className="actions-cell" data-label="Acciones"><div className="request-actions"><button className="ghost" onClick={()=>downloadQuoteJpg(q)}>⬇ JPG</button>{active(q)&&<button className="whatsapp" onClick={()=>whatsapp(q)}>WhatsApp</button>}{active(q)&&<button className="primary" onClick={()=>openAsOrder(q)}>→ Abrir en Nuevo pedido</button>}{active(q)&&<button className="ghost" disabled={busy===String(q.id||q.code)} onClick={()=>approve(q)}>{busy===String(q.id||q.code)?'Guardando…':'✓ Aprobar directo'}</button>}{active(q)&&<button className="danger" disabled={busy===String(q.id||q.code)} onClick={()=>cancel(q)}>Cancelar</button>}</div></td></tr>)}{!visible.length&&<tr><td colSpan="7">No hay presupuestos en esta sección.</td></tr>}</tbody></table></div>
 </>
}
