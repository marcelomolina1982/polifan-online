...
const INDUSTRIAL_URL='https://polifan-cnc-solver-test.onrender.com/nest-v3'
...
function composeIndustrialSvg(placements,partMap){
  ...
  placements.forEach((p,n)=>{
    ...
    if(!parsed)return
    const w=Math.max(1,Number(meta.sourceWidthCm||meta.widthCm||1)*10)
    const h=Math.max(1,Number(meta.sourceHeightCm||meta.heightCm||1)*10)
    ...
    pieces.push(`<g data-industrial-piece="${n}" data-kit="${String(p.kitId||'')}" transform="translate(${x} ${y}) rotate(${angle}) translate(${-trimX} ${-trimY})"><svg width="${w}" height="${h}" viewBox="${parsed.viewBox}" overflow="visible">${cleanInner(parsed.root)}</svg></g>`)
  })
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1220mm" height="580mm" viewBox="0 0 1220 580">${pieces.join('')}</svg>`
}
...
export default function MotorDefinitivo({db,onSave}){
  ...
  async function generateAutomatic(){
    ...
    setBusy(true);setPlans([]);setProgress(`Motor V3 · buscando ${TARGET_COMPLETE}+ kits completos indivisibles`)
    try{
      ...
      if(!r.ok||!data.ok){
        ...
        throw new Error((data.error||`Motor V3 HTTP ${r.status}`)+rejected)
      }
  ...
  async function registerPlan(plan){
    ...
    const batch={id:crypto.randomUUID(),number,date:plan.date||today(),name:`Placa automática V3 ${plan.date||today()}`,status:'En corte',notes:`Motor V3 kits completos + certificador V1.7 · ${plan.units.length} completas · separación ${plan.minGap} mm · conflictos 0 · borde 0`,multiplier:1,items:plan.summary.map(x=>({figure:x.figure,component:'complete',qty:x.qty})),createdAt:new Date().toISOString()}
  ...
  return <>
    <Title title="Generar placas · Motor V3 + Certificador V1.7" sub="Generador por kits completos indivisibles. V1.7 queda como certificador final, el rol en el que ya superó 7/7 placas reales." actions={<button className="primary" disabled={busy||!pending.units.length} onClick={generateAutomatic}>{busy?'Generando…':'Generar una placa'}</button>}/>
    ...
    <div className="panel"><div className="form-grid">
      ...
      <div><small>Arquitectura</small><b className="block big">V3 genera · V1.7 certifica</b></div>
    </div>
    ...
    {progress&&<div className="notice" style={{marginTop:12,marginBottom:0}}><b>{progress}</b><span>V3 prueba combinaciones completas de 10; sólo baja si ninguna combinación completa entra.</span></div>}
    </div>
    <div className="panel table-wrap"><table><thead><tr><th>Placa</th><th>Contenido</th><th>Estado</th><th>Gap certificado</th><th>Conflictos</th><th>Borde</th><th>Ocupación</th><th>Acciones</th></tr></thead><tbody>
      {plans.map(plan=>{const ok=okStatus(plan.status);return <tr key={plan.id}>
        ...
        <td className="row-actions">{ok&&plan.svgText&&<button className="ghost" onClick={()=>downloadSvg('placa-v3-1',plan.svgText)}>Descargar SVG</button>}{ok&&!plan.registered&&<button className="primary" onClick={()=>registerPlan(plan)}>Pasar a corte</button>}{plan.registered&&<span className="green-text"><b>En corte #{plan.batchNumber}</b></span>}</td>
      </tr>})}
      {!plans.length&&<tr><td colSpan="8">Tocá “Generar una placa”. V3 intentará primero varias combinaciones de 10 figuras completas, nunca piezas sueltas.</td></tr>}
    </tbody></table></div>
  </>
}