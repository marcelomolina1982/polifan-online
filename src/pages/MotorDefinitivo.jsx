...
function aliasesForItem(item){
  return [...new Set([item?.productName,item?.modelName,item?.name].map(cleanAlias).map(normalizeFigureKey).filter(Boolean))]
}
function completeComponents(items){
  const simple=items.find(x=>(x.role||'simple')==='simple'&&x.svgText)
  if(simple)return [simple]
  const base=items.find(x=>x.role==='base'&&x.svgText),tapa=items.find(x=>x.role==='tapa'&&x.svgText)
...
function libraryIndex(db){
  ...
  ;(db.svgLibrary||[]).forEach(item=>{
    ...
    const group=groups.get(key);group.items.push(item);aliases.forEach(a=>group.aliases.add(a))
  })
  const list=[...groups.values()].map(g=>({...g,aliases:[...g.aliases]}))
  const exact=new Map()
  list.forEach(group=>group.aliases.forEach(alias=>{if(!exact.has(alias))exact.set(alias,[]);exact.get(alias).push(group)}))
  return {groups:list,exact}
}
function uniqueComplete(groups){
  const out=[]
  groups.forEach(group=>{const comps=completeComponents(group.items);if(comps)out.push({key:group.key,comps})})
...
export default function MotorDefinitivo({db,onSave}){
    ...
    try{
      for(let i=0;i<groups.length;i++){
        ...
        try{
          ...
          const plan={id:crypto.randomUUID(),number:i+1,units,summary:summarizeUnits(units),date:units[0]?.date||today(),registered:false,status:'ERROR',error:error.message,svgText:null,conflicts:'-',border:'-',minGap:'-',seconds:'-'}
          next=[...next,plan];setPlans(next)
        }
      }
  ...
  return <>
    ...
    <div className="panel table-wrap"><table><thead><tr><th>Placa</th><th>Contenido</th><th>Estado</th><th>Gap certificado</th><th>Conflictos</th><th>Borde</th><th>Tiempo</th><th>Acciones</th></tr></thead><tbody>
      {plans.map(plan=>{const ok=okStatus(plan.status);return <tr key={plan.id}><td><b>Placa {plan.number}</b><small className="block">Entrega prioritaria: {plan.date||'sin fecha'}</small><small className="block">{plan.units.length} figuras completas</small></td><td>{plan.summary.map(x=>`${x.figure} × ${x.qty}`).join(', ')}</td><td><b className={ok?'green-text':'red-text'}>{plan.status}</b>{plan.error&&<small className="block red-text">{plan.error}</small>}</td><td><b>{plan.minGap} mm</b></td><td className={Number(plan.conflicts)===0?'green-text':'red-text'}>{plan.conflicts}</td><td className={Number(plan.border)===0?'green-text':'red-text'}>{plan.border}</td><td>{plan.seconds} s</td><td className="row-actions">{ok&&plan.svgText&&<button className="ghost" onClick={()=>downloadSvg(`placa-${plan.number}`,plan.svgText)}>Descargar SVG</button>}{ok&&!plan.registered&&<button className="primary" onClick={()=>registerPlan(plan)}>Pasar a corte</button>}{plan.registered&&<span className="green-text"><b>En corte #{plan.batchNumber}</b></span>}</td></tr>})}
