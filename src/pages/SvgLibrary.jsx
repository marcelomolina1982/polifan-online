...
export default function SvgLibrary({db,onSave}){
  ...
  async function persistScannedCandidate(candidate,{forceMismatch=false}={}){
    ...
    try{
      ...
      if(counterpart){
        ...
        nextLibrary=nextLibrary.map(x=>x.id===counterpart.id?{...x,pairCompatible:forceMismatch?true:pairCompatible,pairMismatch:null,manualSizeAcceptance:forceMismatch||x.manualSizeAcceptance||false}:x)
      }
      const item={id:uid(),modelId,modelName,name:`${modelName} · ${roleLabel(role)}`,role,productId,productName,qtyPerUnit:1,widthCm,heightCm,sourceWidthCm:widthCm,sourceHeightCm:heightCm,originalWidthCm:widthCm,originalHeightCm:heightCm,sizeSource:'svg',sizeLocked:true,allowRotate:true,blockInterior:true,pairCompatible:forceMismatch?true:pairCompatible,pairMismatch:null,manualSizeAcceptance:forceMismatch,acceptedSizeDifference:forceMismatch?pairMismatch:null,svgText:candidate.svgText,svgMeta:{widthCm,heightCm,sourceFile:scanner.fileName,sourceIndex:candidate.sourceIndex,autoAdjustedToPair:false},createdAt:new Date().toISOString()}
  ...
  return <>
    ...
    <div className="notice"><b>Una figura, sus componentes</b><span>La tapa y la base deben compartir el mismo nombre. La app conserva las medidas originales y, si hay una diferencia, te pregunta si querés guardarlas igualmente.</span></div>
    ...
    {scanner&&<div className="svg-scan-overlay" role="dialog" aria-modal="true">
      <div className="svg-scan-dialog panel">
        ...
        <div className="notice"><b>Medidas originales</b><span>La app nunca modifica el tamaño. Si tapa y base no coinciden exactamente, te preguntará si querés guardarlas igualmente.</span></div>
      </div>
    </div>}

    {sizeMismatch&&<div className="svg-scan-overlay" role="dialog" aria-modal="true"><div className="svg-scan-dialog panel"><div className="panel-heading"><div><h2>Las medidas no coinciden exactamente</h2><small>La app conservará ambas piezas con su tamaño original.</small></div><button className="ghost" onClick={()=>setSizeMismatch(null)}>Cerrar</button></div><div className="size-compare-grid"><div className="notice"><b>{roleLabel(sizeMismatch.role)} nueva</b><span>{Number(sizeMismatch.candidateWidthCm).toFixed(3)} × {Number(sizeMismatch.candidateHeightCm).toFixed(3)} cm</span></div><div className="notice"><b>{roleLabel(sizeMismatch.counterpartRole)} guardada</b><span>{Number(sizeMismatch.counterpartWidthCm).toFixed(3)} × {Number(sizeMismatch.counterpartHeightCm).toFixed(3)} cm</span></div></div><div className="notice"><b>Diferencia detectada</b><span>Ancho: {Math.abs(Number(sizeMismatch.candidateWidthCm)-Number(sizeMismatch.counterpartWidthCm)).toFixed(3)} cm · Alto: {Math.abs(Number(sizeMismatch.candidateHeightCm)-Number(sizeMismatch.counterpartHeightCm)).toFixed(3)} cm</span></div><p>Podés elegir otra pieza, subir otro archivo o aceptar esta diferencia. Si elegís <b>Guardar igualmente</b>, la figura quedará completa y se podrá usar normalmente en las placas.</p><div className="title-actions"><button className="ghost" onClick={()=>setSizeMismatch(null)}>Elegir otra pieza</button><button className="ghost" onClick={()=>{setSizeMismatch(null);setScanner(null);requestComponent(scanner.model,scanner.role)}}>Elegir otro archivo</button><button className="primary" disabled={busy} onClick={()=>persistScannedCandidate(sizeMismatch.candidate,{forceMismatch:true})}>Guardar igualmente</button></div></div></div>}
  </>
}
