...
export default function ProductionCalendar({db,onSave,go}){
  ...
  async function toggleClosed(){
    ...
    setClosing(true)
    try{
      const nextDates=selectedClosed
        ? closedDates.filter(date=>date!==selected)
        : [...new Set([...closedDates,selected])].sort()
      const saved=await onSave?.({...db,productionClosedDates:nextDates})
      if(saved?.ok===false)return
      alert(selectedClosed?'Producción reabierta y guardada.':'Producción cerrada y guardada.')
    }catch(error){
      alert('No se pudo guardar el calendario: '+(error?.message||'error de sincronización'))
    }finally{
      setClosing(false)
    }
  }
