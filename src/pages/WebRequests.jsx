...
function customPrice(p,qty){
 const q=Math.max(0,Number(qty)||0)
 if(!q)return 0
 if(p.price100&&q>=100)return q*(Number(p.price100)/100)
 if(p.price12&&q>=12)return q*(Number(p.price12)/12)
 if(p.price6&&q>=6)return q*(Number(p.price6)/6)
 if(p.priceUnit)return q*Number(p.priceUnit)
 if(p.fixedPrice)return q*Number(p.fixedPrice)
 return null
}
...
export default function WebRequests({db,onSave}){
 ...
 function totalFor(items){let regular=0,lights=0,special=0;items.forEach(item=>{const p=productFor(item)||{};const q=Number(item.qty||0);const custom=customPrice(p,q);if(custom!==null)special+=custom;else if(p.category==='Figuras con luces'||p.category==='Palabras con luces')lights+=q;else regular+=q});return regularPrice(regular)+lightPrice(lights)+special}
 ...
 async function createQuote(row){
   const c=detail?.id===row.id?draftCustomer:(row.customer||{})
   const items=((detail?.id===row.id?draftItems:row.items)||[]).filter(i=>Number(i.qty)>0).map(i=>({figure:i.name||i.figure,name:i.name||i.figure,productId:i.productId||'',measure:i.measure||'',qty:Number(i.qty)}))
   if(!items.length)return alert('La solicitud no tiene productos para presupuestar.')
   ...
   const quotes=current?(db.quotes||[]).map(q=>q.id===current.id?quote:q):[...(db.quotes||[]),quote]
   const saved=await onSave({...db,quotes});if(saved?.ok===false)return
   try{await downloadQuoteJpg(quote)}catch(err){console.error('No se pudo generar el presupuesto JPG',err)}
   const {error}=await supabase.from('web_requests').update({status:'Presupuesto enviado'}).eq('id',row.id)
   if(error)console.error('No se pudo actualizar el estado de la solicitud',error)
   setRows(currentRows=>currentRows.map(x=>x.id===row.id?{...x,status:'Presupuesto enviado'}:x));setDetail(d=>d?.id===row.id?{...d,status:'Presupuesto enviado'}:d)
   alert(`${quote.code} guardado y descargado en JPG. Cuando el cliente lo apruebe, usá VENTAS → Presupuestos.`)
 }

 async function saveDraft(){
   if(!detail)return
   const items=draftItems.filter(i=>Number(i.qty)>0).map(i=>({...i,qty:Number(i.qty)}))
   if(!items.length)return alert('La solicitud debe conservar al menos un producto.')
   const customer={...draftCustomer,name:[draftCustomer.firstName,draftCustomer.lastName].filter(Boolean).join(' ').trim()||draftCustomer.name||''}
   const quantity=items.reduce((sum,i)=>sum+Number(i.qty||0),0)
   const estimated_total=totalFor(items)
   const {error}=await supabase.from('web_requests').update({items,quantity,estimated_total,customer}).eq('id',detail.id)
   if(error)return alert('No se pudo editar la solicitud: '+error.message)
   const updated={...detail,items,quantity,estimated_total,customer}
   setRows(current=>current.map(r=>r.id===detail.id?updated:r));setDetail(updated);setDraftItems(items);setDraftCustomer(customer);setEditing(false)
   alert('Solicitud web actualizada. Ya podés confirmarla.')
 }
 ...
 return <>
  <Title title="Solicitudes web" sub="Revisá y editá datos, entrega y productos antes de confirmar el pago y crear el pedido."/>
  ...
  <div className="panel table-wrap"><table><thead><tr><th>Código</th><th>Cliente</th><th>Piezas</th><th>Entrega estimada</th><th>Total</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{visible.map(r=><tr key={r.id}><td><b>{r.code}</b><small className="block">{r.created_at?new Date(r.created_at).toLocaleString('es-AR',{dateStyle:'short',timeStyle:'short'}):''}</small></td><td>{r.customer?.name}<small className="block">{r.customer?.phone}</small></td><td>{r.quantity}</td><td>{r.customer?.estimatedDeliveryStart||r.estimated_from?<>{r.customer?.estimatedDeliveryStart||r.estimated_from} a {r.customer?.estimatedDeliveryEnd||r.estimated_to||r.customer?.estimatedDeliveryStart||r.estimated_from}</>:<b>Fecha a confirmar</b>}</td><td>{money(r.estimated_total)}</td><td>{r.status}</td><td><div className="request-actions"><button className="ghost" onClick={()=>openDetail(r)}>👁️ Ver / editar</button>{isPending(r)&&<button className="ghost quote-button" onClick={()=>createQuote(r)}>🧾 Presupuesto</button>}{isPending(r)&&<button className="primary" onClick={()=>confirmRow(r)}>Confirmar directo</button>}</div></td></tr>)}</tbody></table>{loading&&<p>Cargando…</p>}{!loading&&!visible.length&&<p>{view==='pending'?'No hay solicitudes pendientes.':'Todavía no hay solicitudes en el historial.'}</p>}</div>
  {detail&&<RequestDetail row={detail} items={draftItems} setItems={setDraftItems} customer={draftCustomer} setCustomer={setDraftCustomer} editing={editing} setEditing={setEditing} productFor={productFor} stockFor={stockFor} svgFor={svgFor} onClose={()=>setDetail(null)} onSave={saveDraft} onConfirm={()=>confirmRow(detail)} onRemove={()=>removeRow(detail)} onContact={()=>contact(detail)} onPrint={()=>printRequest(detail)} onQuote={()=>createQuote(detail)}/>} 
 </>
}

function RequestDetail({row,items,setItems,customer,setCustomer,editing,setEditing,productFor,stockFor,svgFor,onClose,onSave,onConfirm,onRemove,onContact,onPrint,onQuote}){
 const c=customer||{};const checks=items.map(item=>({item,product:productFor(item),stock:stockFor(item),svg:svgFor(item)}));const ready=checks.every(x=>x.product&&x.svg.complete);const unique=items.length;const total=items.reduce((s,i)=>s+Number(i.qty||0),0)
 const field=(key,label,type='text')=><label>{label}<input type={type} value={c[key]||''} onChange={e=>setCustomer(v=>({...v,[key]:e.target.value}))}/></label>
 return <div className="web-request-modal-backdrop" role="dialog" aria-modal="true"><div className="web-request-modal panel"><div className="panel-heading"><div><h2>{row.code}</h2><small>{row.created_at?new Date(row.created_at).toLocaleString('es-AR',{dateStyle:'full',timeStyle:'short'}):''}</small></div><button className="ghost" onClick={onClose}>Cerrar</button></div>
  ...
  {editing?<div className="customer-grid">{field('firstName','Nombre')}{field('lastName','Apellido')}{field('phone','WhatsApp')}{field('dni','DNI')}{field('email','Email','email')}<label>Entrega<select value={c.method||'Logística'} onChange={e=>setCustomer(v=>({...v,method:e.target.value}))}><option>Logística</option><option>Retiro en el local</option><option>Vía Cargo</option><option>Correo Argentino</option></select></label>{field('address','Domicilio')}{field('betweenStreets','Entre calles')}{field('locality','Localidad')}{field('district','Partido / Departamento')}{field('province','Provincia')}{field('postalCode','Código postal')}{field('agencyDelivery','Agencia / domicilio')}</div>:<div className="web-request-customer-grid"><div><b>Cliente</b><span>{c.name||[c.firstName,c.lastName].filter(Boolean).join(' ')}</span></div><div><b>WhatsApp</b><span>{c.phone||'—'}</span></div><div><b>DNI</b><span>{c.dni||'—'}</span></div><div><b>Email</b><span>{c.email||'—'}</span></div><div><b>Entrega</b><span>{c.method||'—'} {c.agencyDelivery?`· ${c.agencyDelivery}`:''}</span></div><div><b>Destino</b><span>{c.method==='Retiro en el local'?'Retiro en el local':[c.address,c.locality,c.province,c.postalCode].filter(Boolean).join(' · ')||'—'}</span></div></div>}
