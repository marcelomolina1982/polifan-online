...
export default function WebRequests({db,onSave}){
 ...
 async function confirmRow(r){
  ...
  const used=new Set(freshOrders.map(o=>String(o.number||'').replace(/^0+/,''))),max=Math.max(0,...freshOrders.map(o=>Number(o.number)||0));let nextNum=max+1;while(used.has(String(nextNum)))nextNum+=1
  const next=String(nextNum).padStart(3,'0'),c=r.customer||{},pickup=(c.method||'Logística')==='Retiro en el local';let shippingCost=0,shippingPaid='No corresponde'
  ...
  const saveDb={...freshDb,orders:[...freshOrders,order],clients:upsertClientFromOrder(freshDb.clients||db.clients||[],order),_onlyKeys:['orders','clients']}
  const saved=await onSave(saveDb);if(saved?.ok===false)return alert('El pedido NO se guardó. La solicitud sigue pendiente; recargá y volvé a confirmar.')
