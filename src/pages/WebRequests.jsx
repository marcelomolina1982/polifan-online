...
export default function WebRequests({db,onSave}){
 ...
 <div className="panel table-wrap"><table><thead><tr><th>Código</th><th>Cliente</th><th>Piezas</th><th>Entrega estimada</th><th>Total</th><th>Estado</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.code}</b></td><td>{r.customer?.name}<small className="block">{r.customer?.phone}</small></td><td>{r.quantity}</td><td>{r.customer?.estimatedDeliveryStart||r.estimated_from||'-'} a {r.customer?.estimatedDeliveryEnd||r.estimated_to||'-'}</td><td>{money(r.estimated_total)}</td><td>{r.status}</td><td>{r.status==='Pendiente de pago'&&<button className="primary" onClick={()=>confirmRow(r)}>Confirmar pago y crear pedido</button>}</td></tr>)}</tbody></table>{loading&&<p>Cargando…</p>}{!loading&&!rows.length&&<p>No hay solicitudes.</p>}</div></>
}
