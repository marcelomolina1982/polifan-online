...
export default function Orders({db,onSave,onEdit}){
  ...
  const [printMode,setPrintMode]=useState(2)
  ...
  function printOrders(orders,perPage=2,includeCutList=false){
    ...
    </style></head><body>${cutHtml}<div class="print-grid">${sorted.map(orderTicket).join('')}</div><script>window.onload=()=>window.print()</script></body></html>`)
    win.document.close()
  }
  ...
  return <>
    ...
    <div className="panel bulk-toolbar">
      <div><b>{selected.length} pedido{selected.length===1?'':'s'} seleccionado{selected.length===1?'':'s'}</b><small>Podés imprimir 1, 2 o 4 pedidos por hoja A4.</small></div>
      <div className="bulk-actions">
        ...
        <select className="print-select" value={printMode} onChange={e=>setPrintMode(Number(e.target.value))}>
          <option value={1}>1 por hoja</option><option value={2}>2 por hoja</option><option value={4}>4 por hoja</option>
        </select>
        <button className="primary" disabled={!selected.length} onClick={()=>printOrders(selectedOrders,printMode,false)}>Imprimir seleccionados</button>
        <button className="primary" disabled={!selected.length} onClick={()=>printOrders(selectedOrders,printMode,true)}>Lista de corte + pedidos</button>
