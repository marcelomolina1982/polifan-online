...
function deliveryType(o){
  ...
  if(key.includes('correo argentino')) return 'Correo Argentino'
  if(key.includes('via cargo')||key.includes('vía cargo')) return 'Vía Cargo'
  if(key.includes('otro')) return 'Otro expreso'
  return 'Logística'
}
...
function internalOrderHtml(o){
  ...
  const minutes=pieces*3
  const timeLabel=minutes<60?`${minutes} min`:`${Math.floor(minutes/60)} h ${minutes%60?minutes%60+' min':''}`.trim()
  ...
  return `<section class="internal-order">
    ...
    <div class="internal-body">
      ...
      <div class="internal-kpis">
        <div><small>TOTAL DE PIEZAS</small><b>${pieces}</b><span>PIEZAS</span></div>
        <div class="time-kpi"><small>TIEMPO ESTIMADO</small><b>${esc(timeLabel)}</b><span>3 MIN/PZA</span></div>
