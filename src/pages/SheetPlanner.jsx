...
export default function SheetPlanner({db,onSave}){
  ...
  function loadPending(){
    ...
    for(const row of pending){
      ...
      const components=library.filter(s=>s.productId===product?.id)
  ...
  function addFromLibrary(id){const c=library.find(x=>x.id===id);if(!c)return;const width=num(c.sourceWidthCm||c.widthCm),height=num(c.sourceHeightCm||c.heightCm);if(!width||!height)return alert('Este SVG no declara medidas físicas válidas en mm o cm. Volvé a exportarlo conservando sus medidas reales.');const product=products.find(p=>p.id===c.productId),siblings=library.filter(x=>x.productId===c.productId),totalPerUnit=siblings.reduce((a,x)=>a+num(x.qtyPerUnit,1),0)||1;setItems(v=>[...v,{id:uid(),svgId:c.id,productId:c.productId,figure:product?.name||c.name,name:c.name,width,height,sourceWidth:width,sourceHeight:height,sizeLocked:true,qty:1,unitWeight:1/totalPerUnit,svgText:c.svgText,svgMeta:parseSvg(c.svgText),allowRotate:c.allowRotate!==false,blockInterior:c.blockInterior!==false,color:COLORS[v.length%COLORS.length]}])}
  ...
  return <div className="sheet-planner-page">
    ...
    <div className="planner-layout"><section className="panel planner-items"><div className="panel-heading"><h3>Piezas físicas ({items.reduce((a,x)=>a+num(x.qty),0)})</h3><button className="ghost" onClick={()=>setItems([])}>Limpiar</button></div>
      ...
      {!items.length&&<div className="empty-message">Vinculá SVG al catálogo y presioná “Cargar Para cortar”, o agregalos desde la biblioteca.</div>}
