...
export default function Expenses({db,onSave}){
  ...
  return <>
    ...
    <div className="grid2 cash-history">
      <div className="panel table-wrap operational-table"><h3>Dinero ingresado</h3><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Detalle</th><th>Importe</th><th className="actions-cell">Acciones</th></tr></thead><tbody>{visibleIncomes.map(x=><tr key={x.id}><td>{new Date(x.date+'T12:00:00').toLocaleDateString('es-AR')}</td><td>{x.category}</td><td><b>{x.description}</b><small className="block">{x.notes||''}</small></td><td className="income-amount">+ {money(x.amount)}</td><td className="actions-cell" data-label="Acciones"><div className="row-actions"><button className="ghost" onClick={()=>editIncome(x)}>Editar</button><button className="danger smallbtn" onClick={()=>removeIncome(x)}>Eliminar</button></div></td></tr>)}</tbody></table>{!visibleIncomes.length&&<p className="empty-message">Todavía no registraste ingresos.</p>}</div>
      <div className="panel table-wrap operational-table"><h3>Dinero salido</h3><table><thead><tr><th>Fecha</th><th>Categoría</th><th>Detalle</th><th>Importe</th><th className="actions-cell">Acciones</th></tr></thead><tbody>{visibleExpenses.map(x=><tr key={x.id}><td>{new Date(x.date+'T12:00:00').toLocaleDateString('es-AR')}</td><td>{x.category}</td><td><b>{x.description}</b><small className="block">{x.notes||''}</small></td><td className="expense-amount">- {money(x.amount)}</td><td className="actions-cell" data-label="Acciones"><div className="row-actions"><button className="ghost" onClick={()=>editExpense(x)}>Editar</button><button className="danger smallbtn" onClick={()=>removeExpense(x)}>Eliminar</button></div></td></tr>)}</tbody></table>{!visibleExpenses.length&&<p className="empty-message">Todavía no registraste gastos.</p>}</div>
    </div>
  </>
}
