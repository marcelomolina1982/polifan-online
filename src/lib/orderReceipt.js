...
export function receiptHtml(o){
  ...
  const shipping=quotedShipping(o)
  ...
  return `<div class="receipt">
   ...
   <section class="verify-banner"><b>REVISÁ TODOS TUS DATOS ANTES DE CONFIRMAR</b><span>Este comprobante contiene la información cargada para fabricar y enviar tu pedido.</span></section>
   <section class="grid">
     <div><h3>DATOS PERSONALES</h3><p><b>Nombre:</b> ${esc(o.firstName||'')}</p><p><b>Apellido:</b> ${esc(o.lastName||'')}</p><p><b>Nombre completo:</b> ${esc(o.client||[o.firstName,o.lastName].filter(Boolean).join(' ')||'-')}</p><p><b>DNI:</b> ${esc(o.dni||'-')}</p><p><b>Teléfono:</b> ${esc(o.phone||'-')}</p><p><b>Correo electrónico:</b> ${esc(o.email||'-')}</p></div>
     <div><h3>DATOS DE ENTREGA</h3><p><b>Fecha de entrega:</b> ${esc(deliveryDate(o)||'A coordinar')}</p>${deliveryType==='Retiro en el local'?`<p><b>Tipo de entrega:</b> Retiro en el local</p><p><b>Costo de retiro:</b> Gratis</p><p>Te avisaremos cuando el pedido esté listo para retirar.</p>`:`<p><b>Tipo de entrega:</b> ${esc(deliveryType)}</p><p><b>Modalidad:</b> ${esc(o.agencyDelivery||'-')}</p><p><b>Domicilio:</b> ${esc(o.address||'-')}</p><p><b>Entre calles:</b> ${esc(o.betweenStreets||'-')}</p><p><b>Localidad:</b> ${esc(o.locality||'-')}</p><p><b>Partido / Departamento:</b> ${esc(o.district||'-')}</p><p><b>Provincia:</b> ${esc(o.province||'-')}</p><p><b>Código postal:</b> ${esc(o.postalCode||'-')}</p>`}</div>
   </section>
...
export async function downloadOrderReceiptJpg(order){
  ...
  try{await new Promise(r=>setTimeout(r,180));const canvas=await html2canvas(host.querySelector('.receipt'),{scale:1.5,useCORS:true,backgroundColor:'#ffffff'});const a=document.createElement('a');a.download=`comprobante-pedido-${order.number}.jpg`;a.href=canvas.toDataURL('image/jpeg',.94);a.click()}finally{host.remove()}
}
