...
// Alias de localidades frecuentes cuyo partido permite resolver la zona sin obligar al cliente
// a escribir exactamente el nombre administrativo del municipio.
const LOCALITY_ALIASES={
  ...
  'villa tesei':'hurlingham','william morris':'hurlingham',
  'castelar':'moron','haedo':'moron','el palomar':'moron',
  'remedios de escalada':'lanus','valentin alsina':'lanus','monte chingolo':'lanus',
  'wilde':'avellaneda','sarandi':'avellaneda','dock sud':'avellaneda','gerli':'avellaneda',
  'ingeniero maschwitz':'escobar','belen de escobar':'escobar','matheu':'escobar','garin':'escobar',
  'del viso':'pilar','presidente derqui':'pilar','derqui':'pilar','villa rosa':'pilar',
  'bella vista':'san miguel','muniz':'san miguel',
  'francisco alvarez':'moreno','paso del rey':'moreno','trujui':'moreno',
  'san antonio de padua':'merlo','libertad':'merlo','parque san martin':'merlo',
  'monte grande':'esteban echeverria','luis guillon':'esteban echeverria','el jaguel':'esteban echeverria',
  'tristan suarez':'ezeiza','canning':'ezeiza',
  'adrogue':'almirante brown','burzaco':'almirante brown','longchamps':'almirante brown','claypole':'almirante brown','glew':'almirante brown',
  'bosques':'florencio varela','ingeniero allan':'florencio varela',
  'ranelagh':'berazategui','hudson':'berazategui','platanos':'berazategui',
  'bernal':'quilmes','ezpeleta':'quilmes','don bosco':'quilmes',
...
const zoneForName=raw=>{
  ...
  for(const zone of LOGISTICS_ZONES){
    if(zone.places.some(place=>aliased===normalize(place)))return zone
  }
  return null
}
...
export function resolveLogisticsZone({locality='',district='',province='',postalCode=''}={}){
  ...
  // El partido/departamento tiene prioridad porque es el dato administrativo de cobertura.
