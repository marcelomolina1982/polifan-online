...
export function quoteHtml(q){
  ...
  return `<div class="quote-receipt">
    ...
    <section class="quote-grid">
      ...
      <div><h3>ENTREGA</h3><p><b>Modalidad:</b> ${esc(deliveryType(q))}</p><p><b>Fecha de entrega:</b> ${esc(deliveryDate(q)||'A coordinar')}</p>${q.locality||q.customer?.locality?`<p><b>Localidad:</b> ${esc(q.locality||q.customer?.locality)}</p>`:''}${q.province||q.customer?.province?`<p><b>Provincia:</b> ${esc(q.province||q.customer?.province)}</p>`:''}<p><b>Envío:</b> se coordina por separado.</p></div>
    </section>
