...
export const JOURNEY_EVENTS={CONFIRMED:'confirmed',PRODUCTION_CUT:'production_cut',PACKING:'packing',DISPATCHED:'dispatched',READY_PICKUP:'ready_pickup'}
export const TRACKING_STAGES=[{key:JOURNEY_EVENTS.CONFIRMED,label:'Pedido agendado'},{key:JOURNEY_EVENTS.PRODUCTION_CUT,label:'En producción / corte'},{key:JOURNEY_EVENTS.PACKING,label:'Para embalar'},{key:JOURNEY_EVENTS.DISPATCHED,label:'Despachado'}]

export function customerName(order={}){const full=clean(order.client)||[order.firstName,order.lastName].map(clean).filter(Boolean).join(' ');return full||'cliente'}
...
export function shouldSendJourneyWhatsApp(event){return [JOURNEY_EVENTS.CONFIRMED,JOURNEY_EVENTS.DISPATCHED,JOURNEY_EVENTS.READY_PICKUP].includes(event)}
...
export function journeyMessage(order={},event,{trackingBaseUrl,reviewUrl}={}){
  ...
  if(event===JOURNEY_EVENTS.DISPATCHED){if(deliveryMode(order)==='via-cargo')return `Hola ${nombre} 💜\n\nTu pedido #${numero} ya fue despachado por Vía Cargo hacia la sucursal de destino.${despedida}`;return `Hola ${nombre} 💜\n\nTu pedido #${numero} ya fue despachado y va camino a tu domicilio.${despedida}`}
  return ''
}

export function trackingState(order={},currentEvent=JOURNEY_EVENTS.CONFIRMED){const finalKey=currentEvent===JOURNEY_EVENTS.READY_PICKUP?JOURNEY_EVENTS.DISPATCHED:currentEvent;const activeIndex=Math.max(0,TRACKING_STAGES.findIndex(stage=>stage.key===finalKey));return TRACKING_STAGES.map((stage,index)=>({...stage,state:index<activeIndex?'done':index===activeIndex?'active':'pending'}))}

export function simulateJourneyEvent(outbox=[],order,event,now=new Date().toISOString(),options={}){
  const orderId=clean(order?.id)||`number:${clean(order?.number)}`,key=`${orderId}:${event}`,existing=outbox.find(row=>row.key===key)
  if(existing)return {outbox,entry:existing,duplicate:true}
  const sendWhatsApp=shouldSendJourneyWhatsApp(event)
  const entry={key,orderId,event,recipient:clean(order?.phone||order?.whatsapp),message:sendWhatsApp?journeyMessage(order,event,options):'',sendWhatsApp,status:'simulated',createdAt:now,providerMessageId:null,lastError:null,attachments:event===JOURNEY_EVENTS.CONFIRMED?[{kind:'order-receipt',filename:'pedido.jpg'}]:[],tracking:trackingState(order,event)}
  return {outbox:[...outbox,entry],entry,duplicate:false}
}
export function eventForFinalAction(order={}){return deliveryMode(order)==='pickup'?JOURNEY_EVENTS.READY_PICKUP:JOURNEY_EVENTS.DISPATCHED}
