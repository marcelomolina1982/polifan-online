...
export default function AppV2(){
  ...
  const loadedRef=useRef(new Set(cached.keys||[])),baselineRef=useRef(initial),requestRef=useRef(0),saveRef=useRef(false)
  ...
  async function saveData(next){if(saveRef.current){alert('Ya se está guardando otro cambio. Esperá a que termine antes de continuar.');return{ok:false,busy:true}}if(loadWarning){alert('Por seguridad no se puede guardar mientras los datos no estén sincronizados. Recargá la app y esperá a que indique Sincronizado.');return{ok:false,safeMode:true}}const keys=changedKeys(db,next);if(!keys.length)return{ok:true,data:db};saveRef.current=true;setSaving(true);try{const latestResult=await loadV2Sections(keys,{fullCatalog:keys.includes('customerCatalog')&&pageNeedsFullCatalog(page)}),latest={...db,...latestResult.data},baseline=baselineRef.current,patch={},conflicts=[];for(const key of keys){if(recordSections.has(key)){const merged=mergeArray(baseline[key],latest[key],next[key],key);if(!merged.ok)conflicts.push(`${key} (${merged.conflicts.length})`);else patch[key]=merged.value}else if(stable(latest[key])!==stable(baseline[key])&&stable(latest[key])!==stable(next[key]))conflicts.push(key);else patch[key]=next[key]}const blockingConflicts=conflicts;if(blockingConflicts.length){alert('Otra sesión modificó exactamente el mismo dato: '+blockingConflicts.join(', ')+'. Recargá esa sección y repetí sólo ese cambio.');return{ok:false,conflict:true}}await patchV2Sections(patch,session?.user?.id,latestResult.revisions);const confirmed={...db,...patch};baselineRef.current={...baselineRef.current,...patch};setDb(confirmed);writeCache({keys:[...loadedRef.current],data:Object.fromEntries([...loadedRef.current].map(k=>[k,confirmed[k]]))});return{ok:true,data:confirmed}}catch(error){console.error(error);alert('No se pudo guardar: '+error.message);return{ok:false,error}}finally{saveRef.current=false;setSaving(false)}}
